...
#   single end-to-end canary chain: submit → SUBMIT_REQUESTED persisted →
...
"""B6 end-to-end live-submit canary chain.
...
_CANARY_ORDER_ID = "venue-canary-ord-001"
_POSITION_ID = "canary-pos-001"
_COMMAND_PREFIX = "canary-cmd-"
...
def _insert_snapshot(conn: sqlite3.Connection, token_id: str) -> str:
    ...
    insert_snapshot(
        ...
        ExecutableMarketSnapshot(
            ...
            gamma_market_id="gamma-canary",
            event_id="event-canary",
            event_slug="event-canary",
            condition_id="condition-canary",
            question_id="question-canary",
...
# ---------------------------------------------------------------------------
# The canary chain test
# ---------------------------------------------------------------------------

def test_live_submit_canary_chain(monkeypatch, tmp_path):
    """End-to-end canary: submit → durability → crash → reconcile (no dupe).
    ...
    def _place_limit_order_side_effect(**kwargs):  # noqa: ANN001
        return _final_submit_result(bound, order_id=_CANARY_ORDER_ID)
    ...
    assert result.order_id == _CANARY_ORDER_ID
    ...
    # venue_order_id matches _CANARY_ORDER_ID but has no SUBMIT_ACKED event.
    ...
    insert_command(
        ...
        envelope_id=crash_env_id,
        position_id="canary-crash-pos-001",
        decision_id="canary-crash-dec-001",
        idempotency_key=crash_idem_key,
        intent_kind="ENTRY",
        market_id="gamma-canary",
