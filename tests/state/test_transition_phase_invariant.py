...
# BRIDGE RETIREMENT (docs/rebuild/quarantine_excision_2026-07-11.md, T5): the
# mixed-epoch downgrade helper that rebuilt position_current/position_events
# with the pre-migration CHECK (to let a test seed a legacy phase='quarantined'
# row) was retired here along with its two consumers below
# (test_entry_authority_quarantined_position_can_transition_to_pending_exit,
# test_chain_absent_confirmed_position_cannot_transition_to_pending_exit) —
# both existed only to pin src.state.canonical_write.transition_phase's now-
# deleted phase_before=='quarantined' preservation branch. The scenario is
# structurally impossible on a real DB post-migration (the CHECK constraint
# rejects the literal) and transition_phase no longer special-cases it.
