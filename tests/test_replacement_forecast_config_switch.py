...
# Purpose: Protect replacement forecast live-authority config switch planning.
# Reuse: Run before changing replacement forecast runtime flag wiring.
# Authority basis: Operator-directed live-only switch for Open-Meteo ECMWF IFS 9km plus AIFS sampled-2t.
...
def test_config_switch_plan_targets_live_authority_flags() -> None:
    ...
    assert dict(plan.target_flags)[TRADE_AUTHORITY_FLAG] is True
...
def test_live_authority_config_switch_ignores_legacy_promotion_evidence() -> None:
...
def test_live_authority_config_switch_accepts_capital_objective_evidence() -> None:
    plan = build_replacement_forecast_live_authority_config_switch_plan(
        _settings(),
        promotion_evidence=_weak_promotion_evidence(),
...
def test_live_authority_config_switch_targets_full_live_authority(tmp_path) -> None:
    settings_path = tmp_path / "settings.json"
    settings_path.write_text(json.dumps(_settings()), encoding="utf-8")
...
def test_config_switch_can_add_missing_replacement_flags(tmp_path) -> None:
    ...
    for key, value in TARGET_LIVE_AUTHORITY_FLAGS.items():
...
def test_config_switch_blocks_non_bool_flags_before_writing(tmp_path) -> None:
    settings_path = tmp_path / "settings.json"
    before = _settings({TRADE_AUTHORITY_FLAG: "true"})
...
def test_config_switch_cli_defaults_to_read_only_plan(tmp_path) -> None:
    ...
    assert payload["policy_status_after"] == "LIVE_AUTHORITY"
...
def test_config_switch_cli_apply_updates_temp_settings_only(tmp_path) -> None:
    ...
    assert settings_payload["feature_flags"][TRADE_AUTHORITY_FLAG] is True
