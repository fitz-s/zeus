...
def test_celsius_integer_market_bin_uses_settlement_rounding_window() -> None:
    extraction = extract_aifs_sampled_2t_localday(
        (
            AifsInstantSample("pf001", _dt(2026, 6, 9, 12), 28.4769, "C"),
            AifsInstantSample("pf002", _dt(2026, 6, 9, 12), 28.5000, "C"),
            AifsInstantSample("pf003", _dt(2026, 6, 9, 12), 27.4999, "C"),
        ),
        city_timezone="UTC",
        target_local_date=date(2026, 6, 9),
        source_cycle_time=_dt(2026, 6, 7, 0),
    )
    bins = (
        AifsTemperatureBin("26C_or_below", upper_c=26.0, center_c=25.0),
        AifsTemperatureBin("27C", lower_c=27.0, upper_c=27.0),
        AifsTemperatureBin("28C", lower_c=28.0, upper_c=28.0),
        AifsTemperatureBin("29C", lower_c=29.0, upper_c=29.0),
        AifsTemperatureBin("30C_or_higher", lower_c=30.0, center_c=31.0),
    )

    result = build_aifs_sampled_2t_bin_probabilities(extraction, metric="high", bins=bins)

    assert result.member_assignments["pf001"] == "28C"
    assert result.member_assignments["pf002"] == "29C"
    assert result.member_assignments["pf003"] == "27C"


def test_celsius_display_bin_with_fahrenheit_source_rounds_in_fahrenheit_first() -> None:
    extraction = extract_aifs_sampled_2t_localday(
        (
            AifsInstantSample("pf001", _dt(2026, 6, 9, 12), 28.4769, "C"),
            AifsInstantSample("pf002", _dt(2026, 6, 9, 12), 28.62, "C"),
            AifsInstantSample("pf003", _dt(2026, 6, 9, 12), 27.49, "C"),
        ),
        city_timezone="UTC",
        target_local_date=date(2026, 6, 9),
        source_cycle_time=_dt(2026, 6, 7, 0),
    ...
    bins = (
        AifsTemperatureBin("27C_or_below", upper_c=27.0, center_c=26.0, display_unit="C", settlement_unit="F"),
        AifsTemperatureBin("28C", lower_c=28.0, upper_c=28.0, display_unit="C", settlement_unit="F"),
        AifsTemperatureBin("29C_or_higher", lower_c=29.0, center_c=30.0, display_unit="C", settlement_unit="F"),
    )

    result = build_aifs_sampled_2t_bin_probabilities(
        extraction,
        metric="high",
        bins=bins,
    ...
    assert result.member_assignments["pf001"] == "28C"
    assert result.member_assignments["pf002"] == "29C_or_higher"
    assert result.member_assignments["pf003"] == "27C_or_below"


def test_hko_city_keeps_celsius_oracle_truncate_not_fahrenheit_half_up() -> None:
    semantics = SettlementSemantics.for_city(cities_by_name["Hong Kong"])
    assert semantics.measurement_unit == "C"
    assert semantics.rounding_rule == "oracle_truncate"
    assert semantics.round_single(28.9) == 28.0

    extraction = extract_aifs_sampled_2t_localday(
        (
            AifsInstantSample("pf001", _dt(2026, 6, 9, 12), 28.9, "C"),
            AifsInstantSample("pf002", _dt(2026, 6, 9, 12), 29.0, "C"),
        ),
    ...
    bins = (
        AifsTemperatureBin("27C_or_below", upper_c=27.0, center_c=26.0, rounding_rule="oracle_truncate"),
        AifsTemperatureBin("28C", lower_c=28.0, upper_c=28.0, rounding_rule="oracle_truncate"),
        AifsTemperatureBin("29C_or_higher", lower_c=29.0, center_c=30.0, rounding_rule="oracle_truncate"),
    )
