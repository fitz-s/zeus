...
def test_live_adapter_passes_entry_pause_into_global_batch(monkeypatch):
    trade = sqlite3.connect(":memory:")
    forecast = sqlite3.connect(":memory:")
    topology = sqlite3.connect(":memory:")
    world = sqlite3.connect(":memory:")
    ...
    monkeypatch.setattr(
        ...
        lambda conn: (
            "external:manual_command"
            if conn is trade
            else pytest.fail("pause authority must use the trade connection")
        ),
    )
    adapter = era.event_bound_live_adapter_from_trade_conn(
        trade,
    ...
    assert result.events == (event,)
    assert captured["new_entry_block_reason"] == (
        "entries_paused:external:manual_command"
    )
...
def test_global_batch_reauctions_once_on_full_universe_curve_drift(monkeypatch):
    ...
    books = iter(
        (
            _global_test_book("book-1", price="0.41", captured_at=decision_at),
            _global_test_book(
                "book-2",
                price="0.42",
                captured_at=decision_at + _dt.timedelta(seconds=10),
            ),
        )
    )
    ...
    assert calls == {
        ...
        "deadlines": [
            ...
            decision_at + _dt.timedelta(seconds=40),
        ],
        "venue": 1,
    }
    assert result.winner_event_id == event_b.event_id
...
def test_global_batch_entry_pause_without_holdings_skips_scope_and_books(monkeypatch):
    decision_at = _dt.datetime(2026, 7, 16, 18, 0, tzinfo=_dt.timezone.utc)
    event = _global_scope_event(city="Alpha", source_run_id="run-a")
    calls = {"scope": 0, "prepare": 0, "books": 0}
    ...
    def scan(**_kwargs):
        calls["scope"] += 1
        raise AssertionError("entry-only pause must stop before scope scan")
    ...
    result = global_batch_runtime.process_current_global_batch(
        ...
        trade_conn=object(),
        payload_reader=lambda current: json.loads(current.payload_json),
        prepare_event=lambda *_: calls.__setitem__(
            "prepare", calls["prepare"] + 1
        ),
        actuate_winner=lambda *_: pytest.fail("pause must not actuate"),
        stamp_receipt=lambda receipt: receipt,
        venue_submit_count=lambda: 0,
        current_execution=lambda *_: object(),
        current_time_provider=lambda: decision_at,
        current_book_epoch_provider=lambda *_: calls.__setitem__(
            "books", calls["books"] + 1
        ),
        new_entry_block_reason="entries_paused:test",
    )

    assert calls == {"scope": 0, "prepare": 0, "books": 0}
    assert result.winner_event_id is None
    assert result.venue_submit_count == 0
    assert result.receipts[event.event_id].reason == (
...
def test_global_batch_entry_pause_scopes_to_holdings_and_preserves_sell(monkeypatch):
    decision_at = _dt.datetime(2026, 7, 10, 8, 0, tzinfo=_dt.timezone.utc)
    held_event = _global_scope_event(city="Alpha", source_run_id="run-a")
    entry_event = _global_scope_event(city="Beta", source_run_id="run-b")
    scanned_scope = current_global_auction_scope_from_events(
        (held_event, entry_event),
        captured_at_utc=decision_at,
    )
    held_family, entry_family = scanned_scope.family_keys
    witness = SimpleNamespace(
        family_key=held_family,
        captured_at_utc=decision_at,
        posterior_identity_hash="run-a",
        witness_identity="q-run-a",
    )
    ...
    selected = SimpleNamespace(
        decision=SimpleNamespace(candidate=sell, no_trade_reason=None),
        winner_event_id=held_event.event_id,
        actuation=SimpleNamespace(
            decision=SimpleNamespace(candidate=sell),
            actuation_identity="sell-actuation",
            wealth_witness_identity="wealth-1",
        ),
    )
    base_asset = _global_test_book("book-held", price="0.40").assets[0]
    asset = SimpleNamespace(
        **(
            vars(base_asset)
            | {
    ...
    book = SimpleNamespace(
        witness_identity="book-held",
        captured_at_utc=decision_at,
        max_age=_dt.timedelta(seconds=30),
        assets=(asset,),
        sell_assets=(asset,),
    )
    calls = {"prepared": [], "books": [], "venue": 0}
    held_payload = json.loads(held_event.payload_json)
    ...
    def select(_prepared, **kwargs):
        assert kwargs["current_scope"].family_keys == (held_family,)
        policy = kwargs["candidate_policy_rejection_resolver"]
        assert policy(buy) == "entries_paused:test"
    ...
    def prepare(current, _at):
        calls["prepared"].append(current.event_id)
        assert current.event_id == held_event.event_id
        return EventSubmissionReceipt(
            False,
            current.event_id,
            current.causal_snapshot_id,
    ...
    def books(probabilities, _at):
        calls["books"].append(tuple(probabilities))
        return probabilities, book

    def actuate(_event, actuation, _at, token, _authority):
        assert actuation.decision.candidate is sell
        assert token == "sell-binding"
        calls["venue"] += 1
        return EventSubmissionReceipt(
            True,
            held_event.event_id,
            held_event.causal_snapshot_id,
            proof_accepted=True,
            side_effect_status="EXIT_SUBMITTED",
        )
    ...
    assert calls["prepared"] == [held_event.event_id]
    assert calls["books"] == [(held_family,)]
    assert calls["venue"] == 1
    assert result.winner_event_id == held_event.event_id
    assert result.venue_submit_count == 1
    assert result.receipts[held_event.event_id].side_effect_status == (
        "EXIT_SUBMITTED"
    )
    assert result.receipts[entry_event.event_id].reason.startswith(
        "GLOBAL_NOT_SELECTED:"
    )
    assert entry_family not in calls["books"][0]
