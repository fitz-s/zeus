...
# Reuse: Run before wiring Open-Meteo ECMWF IFS 9km replacement posterior into readers or event reactor.
...
def _deps(
    *,
    ...
    posterior_version = HIGH_DATA_VERSION if metric == "high" else LOW_DATA_VERSION
    deps = []
    for role, source_id, product_id, data_version in (
        ...
        ("soft_anchor_posterior", SOURCE_ID, PRODUCT_ID, posterior_version),
    ):
...
def test_replacement_readiness_is_live_when_all_dependencies_are_available_before_decision() -> None:
    decision = _decision()
    ...
    assert decision.provenance_json["runtime_layer"] == LIVE_RUNTIME_LAYER
...
def test_replacement_readiness_validates_dependency_identity_by_metric() -> None:
    ...
    assert dependencies["soft_anchor_posterior"]["data_version"] == LOW_DATA_VERSION
...
def test_replacement_readiness_has_no_experiment_dependency_role() -> None:
    decision = _decision()
