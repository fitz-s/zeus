...
# Purpose: Antibody — the non-day0 HIGH validate_high_distribution log-only path was REMOVED
#   2026-06-14 (gate-mass collapse). This file now pins (a) the log-only path stays gone, and
#   (b) the day0 HIGH hard gate still blocks.
# Authority basis: docs/archive/2026-Q2/operations_historical/P0_FORECAST_EXTREMA_AUTHORITY_2026-05-22.md §P0-D
"""P0-D: day0 HIGH hard gate + non-day0 log-only removal antibody.

End-to-end tests driving evaluate_candidate (the real production entry point).

Covers:
  - Non-day0 pathological distribution → NO [PROBABILITY_SANITY_REMOVED_LOG_ONLY] log and no
    block on it (the log-only path was deleted 2026-06-14; per-edge
...
def _patch_non_day0_path(monkeypatch) -> None:
    """Patch evaluate_candidate through to the removed log-only block.

    validate_high_distribution is patched to return (False, "REMOVED_LOG_FIRED")
    so we can confirm the removed log fires without relying on real distribution logic.
    """
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **kw: (False, "REMOVED_LOG_FIRED"),
    )
...
# ---------------------------------------------------------------------------
# P0-D non-day0 log-only path REMOVED 2026-06-14 — antibody: stays gone
# ---------------------------------------------------------------------------

def test_non_retired_day0_no_submit_scope_removed_no_log_no_block(monkeypatch, caplog):
    """The non-day0 HIGH validate_high_distribution log-only path (log-only) was DELETED
    ...
    [PROBABILITY_SANITY_REMOVED_LOG_ONLY] and must NOT block on it. Non-day0 HIGH coverage lives
    in the per-edge probability_edge_bin_sanity gate, not this removed family log-only path.
    """
    ...
    removed_log_lines = [
        ...
        if "[PROBABILITY_SANITY_REMOVED_LOG_ONLY]" in r.message
    ]
    assert not removed_log_lines, (
        f"non-day0 probability-sanity log-only telemetry must stay removed; got: {removed_log_lines}"
    )
    ...
    for d in decisions:
        assert d.rejection_reason_enum != NoTradeReason.PROBABILITY_SANITY_GATE, (
            "non-day0 path must not block on the removed log-only path"
        )
...
def test_day0_high_hard_gate_still_blocks(monkeypatch):
    ...
    validate_high_distribution returns (False, "REMOVED_LOG_FIRED") in both paths;
