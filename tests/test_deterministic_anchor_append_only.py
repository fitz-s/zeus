...
def test_deterministic_anchor_identical_identity_reuses_existing_row() -> None:
    conn = _conn()
    first = materialize_replacement_forecast_live(conn, _request())
    second = materialize_replacement_forecast_live(conn, _request())
...
def test_deterministic_anchor_append_new_source_run_not_do_nothing() -> None:
    conn = _conn()
    first = materialize_replacement_forecast_live(conn, _request())
    second = materialize_replacement_forecast_live(conn, _request(openmeteo_source_run_id="om9-run-new"))
