...
share its model_key with a low-track model. Verified via platt_models UNIQUE key.
Also tests save_platt_model requires metric_identity.
"""
...
class TestPlattModelV2FamilyIsolation:
    ...
    def test_save_platt_model_missing_metric_identity_raises_type_error(self):
        """R-4D pre-gate: save_platt_model must require metric_identity — no default."""
        from src.calibration.store import save_platt_model  # noqa: F401 — must exist
    ...
    def test_high_and_low_models_have_different_model_keys(self):
        """R-4D: save_platt_model must not produce the same model_key for different tracks."""
        ...
        rows = conn.execute(
            "SELECT model_key, temperature_metric FROM platt_models"
        ).fetchall()
        assert len(rows) == 2, (
            f"Expected 2 platt_models rows (one high, one low), got {len(rows)} (R-4D)"
        )
    ...
    def test_high_track_model_has_correct_data_version(self):
        ...
        row = conn.execute(
            "SELECT temperature_metric, data_version FROM platt_models"
        ).fetchone()
    ...
    def test_model_row_has_no_city_or_target_date_column(self):
        ...
        assert "city" not in cols, "platt_models must not have 'city' column (R-4D/R-N)"
        assert "target_date" not in cols, "platt_models must not have 'target_date' column (R-4D/R-N)"
    ...
    def test_refit_twice_leaves_exactly_one_active_row(self):
        ...
        active = conn.execute(
            "SELECT COUNT(*) FROM platt_models WHERE temperature_metric='high' AND is_active=1"
        ).fetchone()[0]
        assert active == 1, (
        ...
        row = conn.execute(
            "SELECT param_A FROM platt_models WHERE temperature_metric='high' AND is_active=1"
        ).fetchone()
    ...
    def test_high_refit_does_not_touch_low_track_rows(self):
        """4D metric-scope guard: fitting high must not modify any low-track platt_models row."""
        from src.calibration.store import deactivate_model, save_platt_model
        from src.types.metric_identity import HIGH_LOCALDAY_MAX, LOW_LOCALDAY_MIN
        ...
        low_row = conn.execute(
            "SELECT is_active, param_A FROM platt_models WHERE temperature_metric='low'"
        ).fetchone()
...
class TestBucketKeyEmfParity:
    ...
    must be byte-identical to the model_key saved by save_platt_model for both
    ...
    ':emf=<family>' even when family=='none', but save_platt_model omits the
    ...
    def _save_and_get_model_key(self, conn, family: str) -> str:
        ...
        conn.commit()
        row = conn.execute(
            "SELECT model_key FROM platt_models WHERE is_active=1 ORDER BY rowid DESC LIMIT 1"
        ).fetchone()
        assert row is not None
        return row[0]

    def test_none_family_bucket_key_matches_saved_model_key(self):
        """bucket_key formula for family='none' must equal save_platt_model model_key."""
        from src.types.metric_identity import HIGH_LOCALDAY_MAX
        conn = self._make_conn()
        ...
        assert ":emf=" not in saved_key, (
            f"save_platt_model must omit ':emf=' for family='none', got {saved_key!r}"
        )
    ...
    def test_full_transport_family_bucket_key_matches_saved_model_key(self):
        ...
        assert saved_key.endswith(":emf=full_transport_v1"), (
            f"save_platt_model must end with ':emf=full_transport_v1' for that family, "
            f"got {saved_key!r}"
        )
