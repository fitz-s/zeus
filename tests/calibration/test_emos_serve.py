...
# ---------------------------------------------------------------------------
# FIX B — write-boundary staleness antibody tests
# Rows with decision_time > 2 days old must be rejected at write time.
# ---------------------------------------------------------------------------

class TestWriteBoundaryStalenesReject:
    """_write_emos_shadow_ledger must skip rows whose decision_time is stale.

    Invariant: a row written with decision_time 10 days ago is NOT appended
    to the ledger.  A row written with decision_time = now IS appended.

    Tests use a real (isolated) call to append_ledger via the env-var seam
    so the staleness check is exercised end-to-end through the adapter.
    """

    def _call_write_with_decision_time(self, decision_time, tmp_ledger_path, monkeypatch):
        """Call _write_emos_shadow_ledger with a controlled decision_time.

        Returns the number of lines written to tmp_ledger_path.
        Uses a minimal fixture family/snapshot/analysis to exercise the full
        append path without needing a live reactor.
        """
        import json
        import os
        ...
        monkeypatch.setenv("ZEUS_EMOS_LEDGER_PATH", str(tmp_ledger_path))

        # Minimal mock objects
        candidate = MagicMock()
        candidate.condition_id = "test_cond"
        candidate.bin.low = 25.0
        candidate.bin.high = 30.0
        candidate.bin.label = "25-30"
        candidate.bin.unit = "C"

        family = MagicMock()
        family.city = "Chicago"
        family.target_date = "2026-05-25"
        family.metric = "high"
        family.family_id = "test_family"
        family.candidates = [candidate]

        # 51 degC members ~28°C
        members_json = json.dumps([28.0] * 51)
        snapshot = {
        ...
        # event.payload_json must be a valid JSON string (json.loads is called on it)
        event = MagicMock()
        event.payload_json = "{}"

        from src.engine import event_reactor_adapter as adapter

        adapter._write_emos_shadow_ledger(
            event=event,
            family=family,
            snapshot=snapshot,
            analysis=analysis,
            q_by_condition={"test_cond": 0.4},
            decision_time=decision_time,
            lcb_by_direction=None,
            native_costs=None,
        )

        if not tmp_ledger_path.exists():
            return 0
        return sum(1 for line in tmp_ledger_path.read_text().splitlines() if line.strip())

    def test_stale_row_10_days_old_not_written(self, tmp_path, monkeypatch):
        """Row with decision_time 10 days ago must NOT be appended."""
        from datetime import datetime, timedelta, timezone
        decision_time = datetime.now(timezone.utc) - timedelta(days=10)
        ledger = tmp_path / "test_ledger_stale.jsonl"

        n_lines = self._call_write_with_decision_time(decision_time, ledger, monkeypatch)
        assert n_lines == 0, (
            f"Expected 0 lines for 10-day-old decision_time, got {n_lines}"
        )

    def test_fresh_row_is_written(self, tmp_path, monkeypatch):
        """Row with decision_time = now IS appended."""
        from datetime import datetime, timezone
        decision_time = datetime.now(timezone.utc)
        ledger = tmp_path / "test_ledger_fresh.jsonl"

        n_lines = self._call_write_with_decision_time(decision_time, ledger, monkeypatch)
        assert n_lines > 0, (
            f"Expected >0 lines for fresh decision_time, got {n_lines}"
        )
