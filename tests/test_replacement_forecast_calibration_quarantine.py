...
def _request(**overrides) -> ReplacementForecastCalibrationRequest:
    params = {
        ...
        "calibration_method": "soft_anchor_shadow_product_specific",
        "calibration_authority": "SHADOW_PRODUCT_SPECIFIC",
...
def test_product_specific_shadow_calibration_is_allowed_only_as_shadow_evidence() -> None:
    decision = evaluate_replacement_forecast_calibration_quarantine(_request())

    assert decision.allowed is True
    assert decision.status == "SHADOW_PRODUCT_SPECIFIC_ALLOWED"
    assert decision.reason_codes == ("REPLACEMENT_CALIBRATION_SHADOW_PRODUCT_SPECIFIC",)
...
def test_aifs_sampled_2t_cannot_use_period_extrema_calibration() -> None:
    decision = evaluate_replacement_forecast_calibration_quarantine(
        _request(
            ...
            calibration_method="aifs_shadow_product_specific",
        )
    )
...
def test_training_authority_and_non_shadow_authority_are_blocked() -> None:
    decision = evaluate_replacement_forecast_calibration_quarantine(
        _request(training_allowed=True, calibration_authority="LIVE_AUTHORITY")
    )

    assert decision.allowed is False
    assert "REPLACEMENT_CALIBRATION_TRAINING_AUTHORITY_FORBIDDEN" in decision.reason_codes
    assert "REPLACEMENT_CALIBRATION_AUTHORITY_NOT_SHADOW_PRODUCT_SPECIFIC" in decision.reason_codes
