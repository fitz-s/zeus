...
def test_trade_authority_flag_directly_resolves_live_authority() -> None:
    policy = resolve_replacement_forecast_runtime_policy(
        _flags(**{TRADE_AUTHORITY_FLAG: True})
    )

    assert policy.status == LIVE_AUTHORITY_STATUS
    assert policy.reason_codes == ("REPLACEMENT_LIVE_AUTHORITY_FLAG_ENABLED",)
    assert policy.can_initiate_trade is True
    assert policy.can_increase_kelly is False
    assert policy.can_flip_direction is False


def test_disabled_without_trade_authority_has_no_live_powers() -> None:
    policy = resolve_replacement_forecast_runtime_policy(_flags())

    assert policy.status == SAFE_DEFAULT_STATUS
    assert policy.reason_codes == ("REPLACEMENT_TRADE_AUTHORITY_DISABLED",)
    assert policy.can_initiate_trade is False
    assert policy.can_increase_kelly is False
    assert policy.can_flip_direction is False


def test_dangerous_flags_require_trade_authority() -> None:
    policy = resolve_replacement_forecast_runtime_policy(
        _flags(**{KELLY_INCREASE_FLAG: True})
    )

    assert policy.status == "BLOCKED"
    assert "REPLACEMENT_TRADE_AUTHORITY_REQUIRED_FOR_DANGEROUS_FLAGS" in policy.reason_codes
    assert policy.can_initiate_trade is False
    assert policy.can_increase_kelly is False
...
def test_direction_flip_requires_kelly_authority() -> None:
    policy = resolve_replacement_forecast_runtime_policy(
        _flags(**{TRADE_AUTHORITY_FLAG: True, DIRECTION_FLIP_FLAG: True})
    )
