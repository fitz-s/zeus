...
def test_switch_decision_ignores_refit_promotion_without_live_policy() -> None:
    refit_promotion = _decision(refit=_refit(live_promotion=True))

    assert refit_promotion.status == "SHADOW_VETO_ONLY"
    assert refit_promotion.can_initiate_trade is False


def test_switch_decision_admits_direct_new_data_live_authority() -> None:
    ...
    assert missing_refit_promotion.status == "LIVE_AUTHORITY"
