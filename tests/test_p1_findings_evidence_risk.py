...
class TestF4EvidenceTierLifecycle:
    ...
    def test_v27_table_rebuilds_to_admit_schema_version_28_writer_rows(self) -> None:
        # Renamed context: now tests v27→v31 migration (c-epic wave bumped SCHEMA_VERSION 30→31).
        ...
        conn.execute(
            CREATE_EVIDENCE_TIER_ASSIGNMENTS_SQL.replace(
                "DEFAULT 33 CHECK (schema_version IN (25, 26, 27, 28, 29, 30, 31, 32, 33))",
...
class TestF6StrategyKeyCheckMigration:
    ...
    def test_migrate_trade_removes_risk_actions_strategy_check(self) -> None:
        """2026-07-04 live incident: RiskGuard tick failed writing a
        forecast_qkernel_entry gate because live risk_actions still carried
        the founding-4 CHECK; action_type/source/status CHECKs must survive
        the rebuild."""
        conn = sqlite3.connect(":memory:")
        conn.execute(_STALE_RISK_ACTIONS_DDL)
        conn.execute(
            "INSERT INTO risk_actions VALUES (?,?,?,?,?,?,?,?,?,?)",
            ("a1", "opening_inertia", "gate", "true",
             "2026-07-04T00:00:00Z", None, "brier_degraded", "riskguard", 50, "active"),
        )
        conn.commit()
        ...
        conn.execute(
            "INSERT INTO risk_actions VALUES (?,?,?,?,?,?,?,?,?,?)",
            ("a2", "forecast_qkernel_entry", "gate", "true",
             "2026-07-04T01:00:00Z", None, "brier_degraded", "riskguard", 50, "active"),
        )
        conn.commit()
    ...
    def test_migrate_world_removes_risk_actions_strategy_check(self) -> None:
        ...
        conn.execute(
            "INSERT INTO risk_actions VALUES (?,?,?,?,?,?,?,?,?,?)",
            ("a1", "day0_nowcast_entry", "gate", "true",
    ...
    def test_no_trade_events_v27_table_rebuilds_to_admit_schema_version_28(self) -> None:
        # Updated for c-epic wave SCHEMA_VERSION bump 30→31: the test verifies that a
        # v27-only table is rebuilt to accept v28, v29, v30, and v31 rows.
        ...
        # PR1/auth-econ-split: SCHEMA_VERSION advanced to 42 (chain economics cols).
