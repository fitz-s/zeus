...
def test_held_position_family_provider_excludes_closed_phases():
    ...
    conn.executemany(
        ...
        [
            ...
            # post-T5-migration): Seoul/Munich/Lucknow used to carry
            # phase='quarantined' (and Lucknow chain_state=
            # 'entry_authority_quarantined') as examples of disputed-entry
            # rows with real/absent chain risk — both literals retired, DB
            # CHECK no longer admits them. Per REPLACEMENT PHASE LAW such a
            # row now keeps its TRUE phase directly, so these use 'active'
            # with a real chain_state — same inclusion/exclusion assertions.
            ("Seoul", "2026-06-08", "high", 7.0, 7.0, 5.0, 5.0, 5.0, "synced", "active"),
            ("Munich", "2026-06-30", "high", 29.14, 29.14, 21.27, 21.27, 21.27, "chain_absent_confirmed_position_unattributed", "active"),
            ("Lucknow", "2026-06-28", "high", 19.88, 19.88, 0.12, 0.12, 0.12, "synced", "active"),
...
def test_held_condition_scope_excludes_zero_chain_local_ghosts(monkeypatch):
    ...
    conn.executemany(
        ...
        [
            ...
            # BRIDGE RETIREMENT (docs/rebuild/quarantine_excision_2026-07-11.md,
            # post-T5-migration): Munich/Lucknow/Seoul used to carry
            # phase='quarantined' (and Lucknow chain_state=
            # 'entry_authority_quarantined') — both literals retired, DB
            # CHECK no longer admits them. Per REPLACEMENT PHASE LAW these
            # now keep TRUE phase 'active' with a real chain_state — same
            ...
            ("Seoul", "2026-06-28", "high", "quarantine-ghost", "synced", 7.0, "active"),
