...
def test_prepared_confirmed_bridge_ignores_later_rest_confirmation(monkeypatch) -> None:
    from src.events import edli_trade_fact_bridge as bridge
    ...
    ws_fact_id = conn.execute(
        "SELECT MAX(trade_fact_id) FROM venue_trade_facts"
    ).fetchone()[0]
    append_trade_fact(
        conn,
        trade_id="trade-1",
    ...
    candidates = bridge.discover_confirmed_trade_fact_candidates(
        conn,
        trade_schema="main",
        event_schema="main",
    ...
    assert bridge.append_prepared_trade_fact_bridge_evidence(
        conn, prepared, now=NOW + timedelta(minutes=2)
    ) == 1
    payload = json.loads(
        conn.execute(
    ...
    assert payload["source_trade_fact_id"] == ws_fact_id
