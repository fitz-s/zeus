...
| `topology-context-required.yml` | Required merge gate: runs `scripts/ci/check_topology_structural_blockers.py` to evaluate every `blocking_structural` rule in `architecture/topology_enforcement.yaml`. Fails the build only when a `no_override: true` rule fires (workflow_refs_exist, new_test_unregistered, source_rationale_delta_gate, db_table_delta_gate, stdlib_shadowing_gate, expired_override, blocking_rule_lacks_enforcer, context_pack_schema_invalid). Authority: `docs/operations/current/plans/ci_topology_refactor_refined.md` Phase D. |
