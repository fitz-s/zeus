...
def v4_held_sell_reauction_request_is_current_executable(
    request: HeldSellReauctionRequest,
    *,
    ...
    """Validate one queued V4 exact SELL witness for scheduler priority only."""
    ...
    if request.schema_version != HELD_SELL_REAUCTION_V4:
        return False
    if (
        not request.probability_content_identity
        or request.book_state != "EXECUTABLE"
        or request.held_best_bid is None
    ):
        return False
    try:
        bid = float(request.held_best_bid)
        checked_at = now or datetime.now(timezone.utc)
        if checked_at.tzinfo is None:
            return False
        checked_at = checked_at.astimezone(timezone.utc)
        deadline = datetime.fromisoformat(
            request.completion_deadline_at.replace("Z", "+00:00")
        ).astimezone(timezone.utc)
        probability_observed_at = datetime.fromisoformat(
            request.probability_observed_at.replace("Z", "+00:00")
        ).astimezone(timezone.utc)
        bid_observed_at = datetime.fromisoformat(
            request.bid_observed_at.replace("Z", "+00:00")
        ).astimezone(timezone.utc)
    except (AttributeError, TypeError, ValueError):
        return False
    if (
        not math.isfinite(bid)
        or not 0.05 <= bid <= 0.95
        or deadline <= checked_at
        or probability_observed_at > checked_at
        or bid_observed_at > checked_at
        or (checked_at - probability_observed_at).total_seconds() > max_age
        or (checked_at - bid_observed_at).total_seconds() > max_age
    ):
        return False
    try:
        with _held_sell_reauction_lineage_lock(request.scope_identity, path=path):
            ...
            if (
                ...
                or request.scope_identity != latest.scope_identity
                or (
                    request.probability_content_identity
                    != latest.probability_content_identity
                )
            ):
                return False
            if _read_held_sell_reauction_receipt(
                request.request_id,
                path=path,
