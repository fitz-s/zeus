...
|------|---------|---------------|
| `control_plane.py` | Command processing (8 supported commands: pause_entries, resume, tighten_risk, request_status, set_strategy_gate, resolve_review_item, pause_source, resume_source), edge threshold multiplier, strategy gates | Adding new commands changes the external contract |
...
| `heartbeat_supervisor.py` | CLOB V2 venue-heartbeat health and resting-order submit gate | Must use heartbeat supervisor status; do not reintroduce the retired auto-pause tombstone coupling |
