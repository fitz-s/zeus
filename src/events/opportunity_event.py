"""Typed EDLI opportunity event model.

Events are immutable append-only world facts. Consumer progress is tracked in
`opportunity_event_processing`, never by mutating the event row.
"""
...
def assert_live_forecast_has_causal_snapshot(event: OpportunityEvent) -> None:
    if event.event_type == "FORECAST_SNAPSHOT_READY" and not event.causal_snapshot_id:
        raise OpportunityEventValidationError("live forecast decision requires causal_snapshot_id")
