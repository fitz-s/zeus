...
Live forecast authority is the **replacement chain** (`docs/authority/replacement_final_form_2026_06_09.md`; root `AGENTS.md` probability-chain block): RAW model center, **no de-bias shift** (2026-06-18 single-serving-rule §4 — `src/data/bayes_precision_fusion_capture.py` `_raw_instrument`; `eb_bias` has no live caller) → T2 Bayesian precision fusion (`fuse_bayes_precision_posterior`) → settlement-preimage q (`src/calibration/emos.py` `bin_probability_settlement`, q_shape `fused_normal_direct`). The `src/signal/` 51-ENS→P_raw path (`ensemble_signal.py` `analytic_p_raw_vector_from_maxes`) is a telemetry/comparison baseline and receipt provenance only. It must not cap, floor, veto, or otherwise join back into live replacement q without new authority. Edit here for the telemetry baseline only; the live q lives in `src/forecast/` + `src/data/replacement_forecast_*`.
