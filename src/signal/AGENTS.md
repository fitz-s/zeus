...
Live forecast authority is the **replacement chain** (`docs/authority/replacement_final_form_2026_06_09.md`; root `AGENTS.md` probability-chain block): per-model walk-forward de-bias (`src/forecast/bayes_precision_fusion.py` `eb_bias`) → T2 Bayesian precision fusion (`fuse_bayes_precision_posterior`) → settlement-preimage q (`src/calibration/emos.py` `bin_probability_settlement`, q_shape `fused_normal_direct`). The `src/signal/` 51-ENS→P_raw path (`ensemble_signal.py` `analytic_p_raw_vector_from_maxes`) is the **independent legacy baseline / LCB cap** — joined as a floor in `src/engine/event_reactor_adapter.py` (`effective_q_lcb = min(proof.q_lcb_5pct, replacement_hook_result.effective_q_lcb)`), NOT the primary q. Edit here for the baseline only; the live q lives in `src/forecast/` + `src/data/replacement_forecast_*`.
