...
_TERMINAL_POSITIVE_MATCH_FACT_STATES = frozenset({
    "CANCEL_CONFIRMED",
    "EXPIRED",
    "VENUE_WIPED",
})
...
def _is_terminal_positive_match_candidate(candidate: Mapping[str, object]) -> bool:
    command_state = str(candidate.get("state") or "").upper()
    fact_state = str(candidate.get("order_fact_state") or "").upper()
    if command_state not in _TERMINAL_POSITIVE_MATCH_COMMAND_STATES:
        return False
    if fact_state not in _TERMINAL_POSITIVE_MATCH_FACT_STATES:
        return False
    return _positive_decimal_or_none(candidate.get("order_fact_matched_size")) is not None
    ...
    *,
    ...
    for row in _latest_matched_order_fact_candidates(
        ...
        try:
            ...
            point_order_source = str(row.get("order_fact_source") or "REST")
            if callable(get_order):
                try:
                    live_point_order = _venue_order_payload(get_order(order_id))
                    if live_point_order is not None:
                        point_order = live_point_order
                        point_order_source = "REST"
                except Exception as exc:
                    logger.warning(
            ...
            observed_at = (
                str(row.get("order_fact_observed_at") or "")
            ...
            conn.execute(f"SAVEPOINT {sp_name}")
            try:
                if existing_fill_trade_fact_count == 0:
                    append_order_fact(
                        conn,
                        venue_order_id=order_id,
                        command_id=command_id,
                ...
                if str(row.get("intent_kind") or "").upper() == "ENTRY":
                    _append_matched_order_fill_projection(
                        ...
                        fill_price=fill_price,
                        observed_at=observed_at,
                        order_fact_source=point_order_source,
                    )
...
def _review_required_terminal_fak_partial_exit_projection_matches(
    ...
    *,
    ...
    if requested is None or filled <= 0 or filled >= requested or not position_id:
        return False
    row = conn.execute(
        """
        SELECT phase, direction, token_id, no_token_id, shares, chain_state,
    ...
    if not command_token_id or held_token_id != command_token_id:
        return False
...
def reconcile_matched_cancel_review_required_entries(conn: sqlite3.Connection) -> dict:
    ...
    for command in _matched_cancel_review_required_candidates(conn):
        ...
        try:
            ...
            if already_canceled_outcome == "advanced":
                summary["advanced"] += 1
                continue
...
def _cancel_failed_already_canceled_payload(events: list[dict]) -> dict | None:
    latest_event_type, latest_payload = _latest_event_payload(events)
    if latest_event_type != CommandEventType.CANCEL_FAILED.value:
        return None
    ...
    text = " ".join(
        ...
        for value in (
            ...
            cancel_outcome.get("reason"),
            cancel_outcome.get("errorMsg"),
            cancel_outcome.get("errorMessage"),
            cancel_outcome.get("message"),
        )
    ).replace("_", " ").replace("-", " ").lower()
    if "already canceled" in text or "already cancelled" in text:
        return latest_payload
    return None
...
def _terminalize_submit_unknown_fak_no_match_400_if_proven(
    conn: sqlite3.Connection,
    command: dict,
    *,
    occurred_at: str,
    ...
    envelope = _command_envelope(conn, final_envelope_id)
    exception_message = str(latest_payload.get("exception_message") or "")
    from src.venue.polymarket_v2_adapter import _is_polymarket_fak_no_match_error
    ...
    predicates = {
        ...
        "exception_message_fak_no_match_400": (
            _is_polymarket_fak_no_match_error(RuntimeError(exception_message))
        ),
        "final_envelope_command_matches": (
            str(latest_payload.get("final_submission_envelope_command_id") or "")
            == command_id
        ),
        "final_envelope_is_fak": str(envelope.get("order_type") or "").upper() == "FAK",
        "final_envelope_error_matches": (
            _is_polymarket_fak_no_match_error(
                RuntimeError(str(envelope.get("error_message") or ""))
            )
        ),
        "deterministic_order_id_matches": bool(command_order_id)
...
def _reconcile_venue_command_absence_sync(conn: sqlite3.Connection) -> dict:
    """Keep post-submit unknown venue commands fail-closed.

    An EDLI ``Reconciled`` absence event may come from an account snapshot taken
    after the request returned unknown. That snapshot cannot prove that the
    original submit will not appear later, so it cannot authorize replay or
    release the command-side governor. Only a causal order/trade/submit fact may
    terminalize these rows.
    """
    ...
    if rows:
        logger.warning(
            "recovery: keeping %d post-submit unknown venue commands fail-closed; "
            "EDLI account-snapshot absence is not causal side-effect proof",
            len(rows),
        )
    return summary
...
def _reconcile_edli_pre_venue_unknown_thresholds(conn: sqlite3.Connection) -> dict:
    ...
    rows = conn.execute(
        ...
        WHERE proj.pending_reconcile = 1
          AND COALESCE(proj.venue_order_id, '') = ''
          AND COALESCE(json_extract(unknown.payload_json, '$.venue_call_started'), 0) = 0
...
def _apply_edli_post_submit_unknown_absence(
    ...
    del conn, open_read, trade_read
    # A complete account snapshot is still only a point-in-time observation.
    # It cannot prove that an already-started submit will not become visible
    # after NETWORK and before APPLY. Releasing duplicate/cap authority from
    # token-level absence therefore creates a double-submit race. Only a
    # command/order/trade fact causally bound to the original submit may clear
    # this state.
    count = len(rows)
    if count:
        logger.warning(
            "recovery: keeping %d post-submit unknown aggregates fail-closed; "
            "account snapshot absence is not causal side-effect proof",
            count,
        )
    return {"scanned": count, "advanced": 0, "stayed": count, "errors": 0}
...
def _reconcile_edli_post_submit_unknown_absence(
    ...
    """Keep EDLI post-submit unknowns fail-closed until causal proof arrives.
    ...
    for the execution_command_id. A point-in-time account absence snapshot is not
    causal proof that the started submit cannot appear later, so this pass never
    releases replay, duplicate-lock, or cap authority.
    """
    ...
    try:
        ...
        if state == CommandState.SUBMIT_UNKNOWN_SIDE_EFFECT:
            ...
            if fok_killed_payload is not None:
                ...
                return "advanced"
            fak_no_match_payload = _terminalize_submit_unknown_fak_no_match_400_if_proven(
                conn,
                command,
                occurred_at=now,
            )
            if fak_no_match_payload is not None:
                logger.info(
                    "recovery: command %s SUBMIT_UNKNOWN_SIDE_EFFECT -> "
                    "SUBMIT_REJECTED (deterministic FAK no-match 400)",
                    cmd.command_id,
                )
                return "advanced"
...
def reconcile_unresolved_commands(
    ...
    *,
    ...
    """
    if scope not in {"full", "live_tick", "boot_fast", "restart_preflight"}:
...
def _restart_no_venue_exit_retry_candidates(conn: sqlite3.Connection) -> list[dict]:
    if not (
        _table_exists(conn, "venue_commands")
        and _table_exists(conn, "venue_command_events")
        and _table_exists(conn, "venue_order_facts")
        ...
        return []
    pc_cols = _table_columns(conn, "position_current")
    pc_select = ", ".join(f"pc.{col} AS pc_{col}" for col in pc_cols)
    rows = conn.execute(
        f"""
        WITH latest_rejected_exit AS (
            SELECT cmd.command_id,
                   cmd.position_id,
                   cmd.state AS command_state,
                   cmd.venue_order_id,
...
def reconcile_restart_no_venue_exit_retry_projections(conn: sqlite3.Connection) -> dict:
    ...
    for candidate in _restart_no_venue_exit_retry_candidates(conn):
        ...
        try:
            ...
            logger.error(
                "recovery: restart no-venue EXIT retry projection failed for command %s: %s",
                command_id,
                exc,
            )
            summary["errors"] += 1
...
def _reconcile_passes_short_conn(client, summary: dict, started_at: str, *, scope: str = "full") -> None:
    ...
    def _post_submit_unknown_absence_fast_pass():
        """Record no-command post-submit unknowns as intentionally fail-closed.
        ...
        order reads. It performs no network read and no mutation because account
        snapshot absence cannot authorize replay after the submit boundary.
        """
