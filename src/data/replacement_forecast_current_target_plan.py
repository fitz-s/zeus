...
def build_replacement_forecast_current_target_plan(
    ...
    *,
    ...
    try:
        ...
        required = {"market_events", "forecast_posteriors", "readiness_state", "raw_forecast_artifacts"}
        if not required.issubset(tables):
            return ReplacementForecastCurrentTargetPlan(
                status="BLOCKED",
                reason_codes=("REPLACEMENT_CURRENT_TARGET_PLAN_REQUIRED_TABLE_MISSING",),
                target_count=0,
                covered_count=0,
                missing_coverage_count=0,
                can_seed_count=0,
        ...
        if not {"city", "target_date", "temperature_metric", "source_id", "data_version"}.issubset(_columns(conn, "forecast_posteriors")):
