...
def _load_manifests(raw_manifest_dir: Path, *, computed_at: datetime) -> tuple[RawForecastArtifactManifest, ...]:
    ...
    for path in sorted(raw_manifest_dir.glob("*.manifest.json")):
    ...
    try:
        ...
        for target in targets:
            ...
            seed_result = build_replacement_forecast_materialization_seed(
                ...
                openmeteo_payload_json=_resolve_path(openmeteo_payload, base_dir=raw_dir),
                precision_metadata_json=_resolve_path(precision_metadata, base_dir=raw_dir),
                computed_at=computed,
                base_dir=seed_path,
                aifs_samples_json=None if aifs_samples is None else _resolve_path(aifs_samples, base_dir=raw_dir),
                aifs_grib_path=None if aifs_grib is None else _resolve_path(aifs_grib, base_dir=raw_dir),
            )
