...
def _current_target_coverage_inventory(
    ...
    counts = {
        ...
        "future_missing_posterior_count": 0,
        "future_missing_readiness_count": 0,
        "day0_or_past_target_count": 0,
        "day0_or_past_missing_posterior_count": 0,
        "day0_or_past_missing_readiness_count": 0,
    }
    if not forecast_db.exists():
    ...
    future_missing: list[Mapping[str, object]] = []
    for row in plan.rows:
        counts["target_count"] += 1
        posterior_count = int(row.posterior_count)
        readiness_count = int(row.readiness_count)
        day0_or_past = bool(getattr(row, "day0_observed_extreme_required", False))
        is_future = not day0_or_past
        if is_future:
            counts["future_target_count"] += 1
        else:
            counts["day0_or_past_target_count"] += 1
        if posterior_count > 0:
            counts["posterior_covered_count"] += 1
        else:
            counts["missing_posterior_count"] += 1
            if is_future:
                counts["future_missing_posterior_count"] += 1
            else:
                counts["day0_or_past_missing_posterior_count"] += 1
        if readiness_count > 0:
            counts["readiness_covered_count"] += 1
        else:
            counts["missing_readiness_count"] += 1
            if is_future:
                counts["future_missing_readiness_count"] += 1
            else:
                counts["day0_or_past_missing_readiness_count"] += 1
        if posterior_count <= 0 or readiness_count <= 0:
            item = {
                ...
                "day0_observed_extreme_required": day0_or_past,
            }
            missing.append(item)
            if is_future:
                future_missing.append(item)
    if counts["target_count"] <= 0:
        return "NO_CURRENT_TARGETS", counts, ()
    if counts["future_missing_posterior_count"] or counts["future_missing_readiness_count"]:
        return "MISSING_REPLACEMENT_FUTURE_TARGET_COVERAGE", counts, tuple(future_missing[:20])
    return "READY", counts, ()
