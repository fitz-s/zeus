...
THE STRUCTURAL ANTIBODIES (Fitz #1: K structural decisions, not N if-statements):
  ...
  1. DECORRELATED PROVIDER REPS — one representative per physical provider family
     (spec §3): ECMWF anchor + GFS(NOAA) + ICON-global(DWD) + GEM(CMC) + JMA + icon_eu.
     Each is a structurally-decorrelated error source; the fusion's Sigma down-weights
     residual correlation, but we never feed two reps of the SAME provider as if independent.
...
def select_models(
    *,
    ...
    # ---- decorrelated global likelihood reps (spec order) ----
    likelihood_globals = tuple(
        m for m in GLOBAL_LIKELIHOOD_MODELS if m in present and m not in dropped_aliases
    )

    # ---- regional polygon + lead gate ----
