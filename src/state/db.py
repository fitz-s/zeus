...
# ---------------------------------------------------------------------------
# transition_phase — single writer for phase mutations
# ---------------------------------------------------------------------------
#
# Created: 2026-05-18
...
#
# Centralizes phase mutations behind a single writer that atomically emits the
# position_events row alongside the position_current projection. Wraps the
# pre-existing _dual_write_canonical_pending_exit_if_available pattern from
# src/execution/exit_lifecycle.py so every caller producing a pending_exit
# phase change owns both halves of the K-decision (event row + phase column).
#
# Failure mode: conn=None returns False (fail-open posture preserved for live
# runtime; the antibody enforces source-side pairing — that every in-process
# _mark_pending_exit call site is paired with a transition_phase call in the
# same function — independent of DB availability at call time).
