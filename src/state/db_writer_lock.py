...
SQLITE_CONNECT_ALLOWLIST: frozenset[str] = frozenset(
    {
        ...
        "scripts/fit_ens_member_dependence.py",  # read_only_ro_uri: opens forecasts DB via file:...?mode=ro uri; SELECT-only over ensemble_snapshots(ecmwf_ens, serving-side filter) EXISTS settlement_outcomes(VERIFIED); writes state/ens_member_dependence/ens_member_dependence_<as_of>.json + ACTIVE.json pointer only; walk-forward per-metric member-dependence ICC (rho) artifact for the CP effective-n correction (upstream_data_physical consult v2 (f), 2026-07-17)
