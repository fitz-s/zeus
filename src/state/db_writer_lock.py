...
SQLITE_CONNECT_ALLOWLIST: frozenset[str] = frozenset(
    {
        ...
        "scripts/fit_day0_resolver_terminal_residual.py",  # read_only_ro_uri: opens forecasts + world DBs via file:...?mode=ro uri with PRAGMA query_only; SELECT-only over settlements, forecast_posteriors, observation_prints, observation_instants; writes ONLY --out/--eval-out; walk-forward resolver-graded Day0 terminal residual fit (2026-09-24)
        ...
        "scripts/fit_day0_remaining_center_bias.py",  # read_only_ro_uri: opens forecasts DB via file:...?mode=ro uri with PRAGMA query_only=ON; SELECT-only over forecast_posteriors(Day0 carrier provenance) + settlement_outcomes via read_current_settlement_history; writes state/day0_remaining_center_bias.json only; walk-forward Day0 remaining-carrier center shift served by src/calibration/day0_remaining_bias.py (2026-09-24)
        ...
        "scripts/measure_settlement_page_metar_divergence.py",  # read_only_ro_uri: opens forecasts DB via file:...?mode=ro uri; SELECT-only over observations; writes config/wu_metar_divergence.json only (same artifact, measured against the NOAA settlement page instead of a second METAR mirror, 2026-09-17)
        ...
        "scripts/backfill_noaa_wrh.py",                     # already_guarded: writes under db_writer_lock(BULK)
        ...
        # --- current-recipe replay corpus report (2026-09-25) ---
        "scripts/report_probability_replay_corpus.py",  # read_only_ro_uri: opens zeus-world.db, zeus_trades.db and zeus-forecasts.db via file:...?mode=ro uri; SELECT-only; replays archived Day0 carrier states with the current recipe (src/calibration/probability_replay_corpus.py, not wired into the entry path); prints stdout
        ...
        "scripts/migrations/202609_belief_recency_index.py",  # operator_invoked + already_guarded: world-only index/statistics DDL under db_writer_lock(BULK); daemon never imports
        # --- grader-q-from-verified-cert receipt-closure fix backfill (2026-09-03) ---
        "scripts/regrade_unattributable_settlements.py",  # operator_invoked + already_guarded: dry-run (default) opens the world DB via file:...?mode=ro uri, r
        # --- Open-Meteo refetch guard (2026-09-26): provider answer cache ---
        "src/data/openmeteo_response_store.py",  # private cache file state/openmeteo_response_store.db, NOT a canonical DB (no table in db_table_ownership.yaml, no truth): holds provider answers + run-state pins + a unit ledger; every write is one autocommit statement (WAL, busy timeout 2 s) and every failure falls back to the network path, so it needs no writer-class arbitration.ead-only ATTACH of forecasts + trades (via load_settled_positions' own _ensure_trades_attached ro path); --apply opens read-write and writes ONLY through settlement_skill_attribution.persist_grade (the sole settlement_attribution writer) under db_writer_lock(WORLD, BULK), lock acquired/committed/released per chunk; daemon never imports
    }
)
