...
SQLITE_CONNECT_ALLOWLIST: frozenset[str] = frozenset(
    {
        ...
        # --- no_trade_regret_events hindsight enrichment backfill (2026-08-28) ---
        "scripts/enrich_no_trade_regret_outcomes.py",  # operator_invoked + already_guarded: dry-run (default) opens the world DB mode=ro, SELECT-only (plus a read-only-intent ATTACH of forecasts for the settlement/market_events join, never written); --apply writes UPDATE-only under db_writer_lock(WORLD, BULK), lock acquired/released per chunk (precedent: scripts/migrations/202608_decision_log_retention.py); daemon never imports
    }
)
