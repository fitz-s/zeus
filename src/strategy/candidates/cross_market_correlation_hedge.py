...
class CrossMarketCorrelationHedge(BaseStrategyCandidate):
    ...
    def evaluate(
        ...
        *,
        ...
        anchor_source = (
            getattr(metrics, "polymarket_end_anchor_source", "gamma_explicit")
            or "gamma_explicit"
        )
        ...
        def _do_shadow_enter(seq: int) -> None:
            ...
            conn.execute(
                ...
                """,
                (
                    market_slug, temperature_metric, target_date_str, observation_time, seq,
        ...
        if _is_world_db_conn(conn):
            ...
            with db_writer_lock(ZEUS_WORLD_DB_PATH, WriteClass.LIVE):
                seq = allocate_decision_seq(
                    market_slug, temperature_metric, target_date_str, observation_time, conn=conn
                )
