...
@dataclass(frozen=True)
class QuoteBook:
    """Float test-friendly facade over the production Decimal quote book."""
...
def native_executable_cost(book: QuoteBook, *, direction: Direction, shares: float) -> ExecutionPrice:
    native_book = NativeQuoteBook(
        yes_asks=tuple(QuoteLevel(Decimal(str(price)), Decimal(str(size))) for price, size in book.yes_asks),
        no_asks=tuple(QuoteLevel(Decimal(str(price)), Decimal(str(size))) for price, size in book.no_asks),
        yes_bids=tuple(QuoteLevel(Decimal(str(price)), Decimal(str(size))) for price, size in book.yes_bids),
        no_bids=tuple(QuoteLevel(Decimal(str(price)), Decimal(str(size))) for price, size in book.no_bids),
        min_tick_size=Decimal(str(book.tick_size)),
        min_order_size=Decimal(str(book.min_order_size)),
        fee_rate=book.fee_rate,
        neg_risk=book.neg_risk,
    )
    return executable_cost(native_book, direction=direction, shares=Decimal(str(shares)))


def executable_cost(book: NativeQuoteBook, *, direction: Direction, shares: Decimal) -> ExecutionPrice:
    if shares < book.min_order_size:
        raise ExecutableCostError("min order size / min_order_size blocks executable cost")
...
def reject_forbidden_cost_source(source: str) -> None:
    if source in {"midpoint", "display_probability", "last_trade_price", "no_complement"}:
        raise ExecutableCostError(f"{source} is forbidden as executable cost")
