...
from src.strategy.strategy_profile import (
    DEFAULT_STRATEGY_MIN_ENTRY_PRICE,
    try_get as _try_get_strategy_profile,
)
...
CENTER_BUY_ULTRA_LOW_PRICE_MAX_ENTRY = DEFAULT_STRATEGY_MIN_ENTRY_PRICE
...
def evaluate_candidate(
    ...
    # T2 (quarantine excision, docs/rebuild/quarantine_excision_2026-07-11.md):
    ...
    if conn is not None:
        try:
            ...
            # a conn reachable but missing the review_work_items /
            # entry_exposure_obligations tables (e.g. a partial-schema test
            # fixture, or a not-yet-migrated legacy DB), or a duck-typed
            # non-sqlite conn stub (some caller fixtures pass a fake conn
            # object entirely), cannot be queried — skip the DB-backed leg
            # rather than crash the whole evaluation. Broad catch is
            # intentional here (not sqlite3.Error only): this seam consumes
    ...
    if blocked_keys:
        ...
        # never narrows the match (sibling temperature bins for one city/date/
        # metric are one settlement partition regardless of per-bin market/
        # condition id; see src.strategy.family_exclusive_dedup module
        # docstring and src.state.review_work_items.open_items_by_family).
        if any(
            (blocked.city, blocked.target_date, blocked.temperature_metric) == candidate_family_identity
            for blocked in blocked_keys
        ):
            return [_make_rejection_decision(
                rejection_stage="FAMILY_ENTRY_BLOCKED",
                rejection_reasons=[NoTradeReason.FAMILY_ENTRY_BLOCKED.value],
                selected_method=selected_method,
                applied_validations=["family_entry_block"],
                rejection_reason_enum=NoTradeReason.FAMILY_ENTRY_BLOCKED,
                rejection_reason_detail=(
                    f"family={city.name}|{target_date}|{temperature_metric} blocked by "
                    "open ChainOnlyFact/ReviewWorkItem/EntryExposureObligation/"
                    "bridging-quarantined-position"
                ),
            )]
