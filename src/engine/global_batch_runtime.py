    ...
    *,
    ...
    try:
        ...
        full_scope = scan_current_global_auction_scope(
            ...
            restrict_to_families=changed_families,
        )
        log_stage("scope_scan", families=len(full_scope.events_by_family))
        if day0_only:
            _LOG.info(
                "global batch Day0 urgent scope: claimed_events=%d "
                "changed_families=%d held_families=%d families=%d",
                len(event_tuple),
                len(changed_families or ()),
                len(held_families),
