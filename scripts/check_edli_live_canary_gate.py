...
def _db_verification_reasons(conn: sqlite3.Connection, artifact: dict[str, Any]) -> list[str]:
    ...
    for required_type in ("DecisionProofAccepted", "SubmitPlanBuilt", "PreSubmitRevalidated", "LiveCapReserved", "ExecutionCommandCreated"):
    ...
    if not _execution_command_certificate_exists(conn, execution_command_id):
...
def _execution_command_certificate_exists(conn: sqlite3.Connection, execution_command_id: str) -> bool:
