...
USAGE:
    ...
    python scripts/refit_platt.py

    # Isolated write (requires --db plus --no-dry-run --force):
    python scripts/refit_platt.py --db /tmp/platt_stage.db --no-dry-run --force
...
def _revocations_ref_for_conn(conn: sqlite3.Connection) -> str | None:
    """Return a qualified fact_revocations table ref if accessible, or None.

    DIQ packet (docs/rebuild/quarantine_excision_2026-07-11.md): calibration_pairs
    is forecasts-owned (src/state/domains.py) and so is its owner-local
    fact_revocations record — both live in zeus-forecasts.db, the same DB
    refit_platt's connection is already rooted at. No cross-DB ATTACH is
    needed (predecessor decision_integrity_quarantine lived only in the trade
    DB, requiring an ATTACH dance this owner-local reshape removes).
    Returns:
      'fact_revocations' — if the table exists on conn's main schema
      None                — if not accessible (skip exclusion)
    """
    row = conn.execute(
        "SELECT 1 FROM sqlite_master WHERE type='table' AND name='fact_revocations'"
    ).fetchone()
    if row is not None:
...
def _fetch_pairs_for_bucket(
    ...
    # Exclude pairs revoked as non-contributing forecast extrema so corrupt-
    # snapshot training data does not contaminate Platt model fits.
    # calibration_pairs's fact_revocations record is owner-local (forecasts DB,
    # co-located with calibration_pairs) — _revocations_ref_for_conn is a local check.
    _q_ref = _revocations_ref_for_conn(conn)
...
def refit(
    ...
    *,
    ...
    if not buckets:
        stats.refused = True
        print(
            f"refit_platt: no {metric_identity.temperature_metric}-track bucket has at least "
