#!/usr/bin/env python3
"""Initialize replacement forecast shadow tables on a forecast DB."""
...
def _report(
    *,
    ...
    after_set = set(after)
    created = tuple(table for table in REPLACEMENT_SHADOW_TABLES if table not in before_set and table in after_set)
    missing_after = tuple(table for table in REPLACEMENT_SHADOW_TABLES if table not in after_set)
    ...
    return {
        ...
        "db_path": str(db_path),
        "committed": committed,
        "replacement_shadow_tables": list(REPLACEMENT_SHADOW_TABLES),
        "created_tables": list(created),
        "missing_replacement_shadow_tables": list(missing_after),
...
def initialize_replacement_forecast_shadow_schema(
    db_path: Path,
    *,
    commit: bool,
) -> dict[str, object]:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Initialize replacement forecast shadow tables")
    ...
    try:
        report = initialize_replacement_forecast_shadow_schema(args.forecast_db, commit=bool(args.commit))
