...
def _fit_signature_hash(
    ...
    *,
    ...
    TRIBUNAL Findings 1+5 (2026-05-29): ``lead_bucket`` added to payload so signatures
    for different lead combinations cannot collide. ``cycle`` NOT included — cycle is
    already captured via metric (HIGH→0z, LOW→12z) which is already in the payload.
    """
    ...
    for city in cities:
        ...
        for season, months in _iter_seasons_for_city(city):
            for metric in metrics:
                ...
                # TRIBUNAL Findings 1+5: inner loop over lead_bucket.
                # cycle NOT looped: load_bucket_residuals already implements metric-aware
                # cycle selection (HIGH→0z, LOW→12z) internally. Adding a separate cycle
                # loop would require SQL-level cycle filtering to be honest; the metric
                # dimension already encodes cycle preference (see #363 fix).
                for _lb_lo, _lb_hi, _lb_label in LEAD_BUCKET_BOUNDS:
                    bucket_label = f"{city}/{metric}/{season}/{_lb_label}"
                    try:
                        ...
                        # Fix #1 (#363): fit_city_predictive_error must receive the SAME
                        # lead_bucket_filter as the residual probes above — otherwise it
                        # re-loads pooled (all-lead) data internally and every per-bucket
                        # row is fitted on identical pooled residuals (cosmetic re-keying).
                        ...
                        # TRIBUNAL Findings 1+5: error_model_key includes lead_bucket.
                        # cycle NOT in key — metric already encodes cycle preference (#363).
                        # Format: city|metric|season|family|live_dv|lead_bucket
                        error_model_key = (
