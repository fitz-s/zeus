...
This is a forward-only repair tool for live canary aggregates parked in
PENDING_RECONCILE after a venue call started but no order id was returned. It
does not infer absence from local rows. It reads authenticated CLOB open orders
and trades, requires complete reads, requires no matching target token exposure,
then appends Reconciled + CapTransitioned(RELEASED) through the canonical
...
def resolve(*, aggregate_id: str | None, apply: bool) -> int:
    ...
    try:
        ...
        print(f"BEFORE: unresolved_submit={before[0]} reserved_cap={before[1]}")
        print(f"pending aggregates selected: {len(aggregates)}")
    ...
    if not proofs:
        ...
        return 0
    if not apply:
        print("DRY-RUN: re-run with --apply to append Reconciled + CapTransitioned(RELEASED).")
        return 0
    ...
    try:
        with world_write_lock(conn):
            ...
            for proof in proofs:
                ...
                print(f"RESOLVED {agg[:80]}... cap_usage={usage} proof_hash={proof['proof_hash']}")
    finally:
        conn.close()
    ...
    try:
        ...
        ro.close()
    print(f"AFTER: unresolved_submit={after[0]} reserved_cap={after[1]}")
