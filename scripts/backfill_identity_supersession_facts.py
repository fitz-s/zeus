...
#   F2 rework: --apply now REFUSES (fail-closed) unless the live
#   position_events.event_type CHECK admits POSITION_IDENTITY_SUPERSEDED —
#   run scripts/migrations/2026_07_position_identity_supersession_check.py
#   first (dry-run is always safe; it never writes).
# Authority basis: docs/rebuild/local_ledger_excision_2026-07-12.md Round-2
#   delta "六站点特判" duplicate_consolidator entry + wave-1 dual review C2
#   ("Rework F ... and run backfill_identity_supersession_facts over history").
...
COVERAGE REPORT: the printed summary distinguishes "booked" (converted) from
"skipped" (ambiguous/unrecoverable, each with a reason) -- a caller building a
fail-closed gate on this backfill should read `skipped` as the ambiguous-count
marker (wave-1.5 dispatch: "backfill script emits a coverage summary
(converted N, ambiguous M with reasons) -- the reducer will fail-closed on
this marker").
"""
...
_TARGET_EVENT_TYPE = "POSITION_IDENTITY_SUPERSEDED"
...
def _refuse_unless_check_migrated(conn: sqlite3.Connection) -> None:
    """F2 rework: --apply is fail-closed on the live schema. Dry-run is always
    safe (it never writes); --apply against a DB whose position_events CHECK
    does not yet admit POSITION_IDENTITY_SUPERSEDED would insert rows that
    the *next* fresh connection's CHECK still rejects only by accident of
    this script's own connection having already opened -- refuse instead of
    depending on that. Run scripts/migrations/
    2026_07_position_identity_supersession_check.py first."""
    from src.engine.lifecycle_events import position_events_admits_event_type

    if not position_events_admits_event_type(conn, _TARGET_EVENT_TYPE):
        raise SystemExit(
            "REFUSED: this DB's position_events.event_type CHECK does not yet "
            f"admit {_TARGET_EVENT_TYPE!r}. Run scripts/migrations/"
            "2026_07_position_identity_supersession_check.py first, then re-run "
            "this backfill with --apply."
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        _refuse_unless_check_migrated(write_conn)
    ...
    print(
        "COVERAGE: converted="
        f"{written} ambiguous={len(skipped)} "
        f"reasons={sorted({s['reason'] for s in skipped})}"
    )
    return 0
