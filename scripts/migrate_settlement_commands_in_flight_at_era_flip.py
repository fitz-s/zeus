...
# Justification: DML (INSERT/UPDATE) executed via get_forecasts_connection_with_world() — already
# writer-lock contract-compliant through the helper; false-positive is on the bare-sqlite3.connect
# scan only (the only literal "sqlite3.connect(" is in a docstring comment, not a call site).
# Defer reason: helper-routed, not a raw-connect daemon-race risk.
