...
- **Codex-managed worktree closeout:** `$CODEX_HOME/worktrees` is owned by the Codex app, not by a repository reaper. Use disposable managed worktrees by default; do not pin or make one permanent unless the operator explicitly needs a long-lived project. Once a worker's landing is verified and it has no open-PR monitoring obligation, that worker archives its own Codex thread with `set_thread_archived`. Codex snapshots then reclaims the worktree. Never run raw `git worktree remove` against a Codex-managed path. Active, pinned, permanent, dirty, or open-PR worktrees are not closeout candidates.
