...
**Change control [§5].** `live` is the live branch (traded 24/7; commits arrive ONLY by hot-fix cherry-pick or merged PR — never a direct commit/edit; never mutate its checkout — `maintree_git_state_guard`); all work in a worktree, land via hot-fix cherry-pick (urgent money-path defect) or milestone PR (paid review once — batch); a completed Codex-managed worker archives its own thread so Codex snapshots and reclaims its tree; `type(scope): subject`; no destructive git; preserve unrelated dirty work; gates never weakened to land faster.
...
- **Worktree allocation:** each writing agent holds exactly one exclusive, task-scoped role worktree at a time: `data`, `strategy`, `execution`, `governance`, or `hotfix`. Its branch records that role as `<role>/<task-slug>` (for example `data/ens-backfill` or `hotfix/day0-budget`); legacy branch prefixes are not new role assignments. A role names the owned slice, not a permanent directory or reusable branch. Start it from current `live`, keep one writer per role/file slice, and do not pre-create pools or raw `git worktree add`. Managed provisioning is ordinary task setup: an agent must not ask the operator to “allow fork”. Reuse is limited to a clean idle tree already owned by the same role and writer. Explicit authorization is only for an exceptional manual non-managed tree after managed routing is unavailable and its path, purpose, and lifetime are named. At most two code-writing agents run concurrently; read-only investigation needs no writer worktree.
- **All work happens in that linked worktree/branch, is proven there, then lands on live promptly** so the running code reloads and stays coherent. There are exactly two landing lanes:
  - **Hot-fix** — a live defect degrading the money path (wrong settlement, dropped exit, fail-open admission, an unbounded hang). The smallest correct change plus a minimal antibody is handed to the landing authority and verified `git cherry-pick`ed onto `live`. The bar is "restores correct live operation without new risk"; speed is the point.
  - **PR** — every non-hot-fix change that belongs in `live` (functional, milestone, governance, new invariant + antibody tests, schema migration, or gate). Open a PR into `live`, pass the required gates and review, then merge. Paid review fires once — batch related work (≥300 self-authored LOC).
- **Closeout:** after the selected lane is verified, prove that `git cherry live <branch>` has no `+` entries. A clean, inactive, no-PR native worktree may then be removed and its local branch deleted; never remove an active, locked, dirty, pinned, permanent, unlanded, or open-PR tree. `$CODEX_HOME/worktrees` is app-owned: its completed owner archives its own thread with `set_thread_archived`, and Codex snapshots then reclaims the tree. Never run raw `git worktree remove` against a Codex-managed path. The hygiene tool is evidence-only and fails closed when PR state is unavailable.
