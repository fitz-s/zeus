...
`live` is traded continuously. `origin/live` is the single source of truth; the live checkout only fast-forwards to it and restarts. Never edit, commit, amend, reset or switch the live checkout; `maintree_git_state_guard` has no agent bypass.

One task, one worktree: create it from `origin/live` when the task starts and remove it when the task ends. Sub-agents share the parent task's worktree by default; a sub-agent that must edit in parallel gets a child worktree that merges back into the parent branch and is removed by the parent. No worktree outlives its task, and none is kept for a role.

Land by fast-forward: rebase the task branch onto current `origin/live`, prove it, then `git push origin HEAD:live` (fast-forward only; never force), or open a PR into `live` for a review-worthy milestone. A rejected push means another task landed first: rebase, re-prove, push again. The push is the queue; no other integration step exists. Landed means `git merge-base --is-ancestor <sha> origin/live`. A live money-path defect ships as the smallest correct fix plus a behavioral antibody through the same lane. Never weaken freshness/fail-closed gates to land faster. Restart only after the live checkout fast-forwards to the landed tip.

Cleanup is part of the task: before ending, commit or discard everything in the worktree, remove it, and delete its branch once landed. Work that has not landed but must survive stays as a branch (pushed if it backs a PR), never as a worktree. The only reasons to keep a worktree are an open PR under active revision or a running process. A PR belongs to the task that opened it: that task merges it or closes it before ending. `scripts/workspace_converge.py` runs hourly as the backstop: worktrees idle 6 h, branches and PRs idle 7 days, detached omc tmux sessions (screen frozen 24 h or no human input 72 h) and stray live files idle 24 h converge automatically, archived first.

Main thread = integrator; one owner per file/slice. Each repair commit needs a verified defect, behavioral antibody and zero new regressions; remaining findings are fix/refute/defer-with-rationale before landing. Use the lowest fitting model tier; top tier for outcome-deciding money logic. Protocol detail: `docs/operations/current/plans/live_branch_workflow_2026-07-20.md`.
...
Commit as `type(scope): subject`; body for non-obvious rationale/test scope/residual risk. `[skip-invariant]` is only for governance/docs-only baseline bypass. Never stage or commit in the live checkout.

PRs are for complete features, invariant plus antibodies, security gates, covered schema migrations or equivalent milestones. Small verified fixes land directly by fast-forward push; partial work waits on its branch, not in a kept worktree. Batch related work before paid automated PR review. Template: `.github/pull_request_template.md`.
