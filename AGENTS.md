...
- **Worktree allocation:** each writing agent holds exactly one exclusive, task-scoped role worktree at a time: `data`, `strategy`, `execution`, `governance`, or `hotfix`. Its branch records that role as `<role>/<task-slug>` (for example `data/ens-backfill` or `hotfix/day0-budget`); legacy branch prefixes are not new role assignments. A role names the owned slice, not a permanent directory or reusable branch. Start it from current `live`, keep one writer per role/file slice, and do not pre-create pools or raw `git worktree add`; the app allocates managed worktrees unless the operator explicitly authorizes one exceptional manual tree. At most two code-writing agents run concurrently; read-only investigation needs no writer worktree.
