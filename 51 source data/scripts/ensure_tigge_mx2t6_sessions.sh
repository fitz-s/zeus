...
# Full-history resume must not block on pilot-gate files after reboot.
...
STATUS1="$ROOT/tmp/tigge_mx2t6_download_status_a1.json"
STATUS2="$ROOT/tmp/tigge_mx2t6_download_status_a2.json"
STATUS3="$ROOT/tmp/tigge_mx2t6_download_status_a3.json"
STATUS4="$ROOT/tmp/tigge_mx2t6_download_status_a4.json"
STATUS5="$ROOT/tmp/tigge_mx2t6_download_status_a5.json"
...
if d.get("date_from") != "$expected_from" or d.get("date_to") != "$expected_to":
    ...
    raise SystemExit(0)
s=d.get("status")
missing=d.get("missing_tasks")
if s == "complete" or missing == 0:
    print("complete")
else:
    print("incomplete")
PY
...
ensure_session() {
  ...
  if ! should_start_session "$status_path" "$shard_from" "$shard_to"; then
    return 0
  fi

  ECMWF_API_RC_FILE="$rc" \
  STATUS_PATH="$status_path" \
  DATE_FROM="$shard_from" \
  DATE_TO="$shard_to" \
  ...
  REQUIRE_PILOT_GATES="$REQUIRE_PILOT_GATES" \
  "$ROOT/scripts/start_tigge_mx2t6_tmux.sh" "$session"
}

ensure_progress() {
  if tmux has-session -t "$PROGRESS_SESSION" 2>/dev/null; then
    return 0
  fi
  tmux new-session -d -s "$PROGRESS_SESSION" \
    "cd '$ROOT' && $PYTHON_BIN scripts/tigge_region_progress_bars.py --one-line --single-line --expected '$EXPECTED_FILES' --lanes '$lane_count' --bar-width 10 --interval-seconds '$PROGRESS_INTERVAL_SECONDS' --decimals '$PROGRESS_DECIMALS'"
}
