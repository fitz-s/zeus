...
status_paths=(
  "$ROOT/tmp/tigge_mn2t6_download_status_a1.json"
  "$ROOT/tmp/tigge_mn2t6_download_status_a2.json"
  "$ROOT/tmp/tigge_mn2t6_download_status_a3.json"
)
...
if [[ "$ENABLE_ACCOUNT4" == "1" ]]; then
  if [[ -f "$ACCOUNT4_RC" ]]; then
    ...
    sessions+=("$SESSION4")
    status_paths+=("$ROOT/tmp/tigge_mn2t6_download_status_a4.json")
  else
...
if [[ "$ENABLE_ACCOUNT5" == "1" ]]; then
  if [[ -f "$ACCOUNT5_RC" ]]; then
    ...
    sessions+=("$SESSION5")
    status_paths+=("$ROOT/tmp/tigge_mn2t6_download_status_a5.json")
  else
...
for path in "${account_rcs[@]}"; do
  [[ -f "$path" ]] || { echo "missing account rc: $path"; exit 2; }
done
...
for idx in "${!sessions[@]}"; do
  ...
  scripts/start_tigge_mn2t6_tmux.sh "${sessions[$idx]}"
done
