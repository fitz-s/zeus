...
AUDIT_PATH="${AUDIT_PATH:-$ROOT/tmp/tigge_mn2t6_source_manifest_audit.json}"
STATUS_PATH="${STATUS_PATH:-$ROOT/tmp/tigge_mn2t6_download_status.json}"
PILOT_GRIB_INTEGRITY_PATH="${PILOT_GRIB_INTEGRITY_PATH:-$ROOT/tmp/tigge_mn2t6_grib_integrity_pilot.json}"
PILOT_JSON_INTEGRITY_PATH="${PILOT_JSON_INTEGRITY_PATH:-$ROOT/tmp/tigge_mn2t6_json_integrity_pilot.json}"
PILOT_COVERAGE_PATH="${PILOT_COVERAGE_PATH:-$ROOT/tmp/tigge_mn2t6_coverage_pilot.json}"
EXTRA_DOWNLOAD_ARGS=""
if [[ "$DRY_RUN" == "1" ]]; then
...
LOG_PATH="$ROOT/logs/tigge_mn2t6_download_${SESSION_SLUG}_${RUN_TS}.log"
...
if missing or failing:
    print("Pilot gates not satisfied; refusing to start full mn2t6 download.")
    for row in missing:
...
"$PYTHON_BIN" scripts/tigge_mn2t6_download_resumable.py \\
