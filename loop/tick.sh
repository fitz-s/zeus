...
#     2. Snapshot the current git-dirty/untracked path set BEFORE invoking
#        claude, so the post-run check only ever evaluates paths that
#        changed DURING this tick — operator-dirty files present before the
#        tick started are never touched, restored, or reported.
...
#     4. Post-run allowlist diff check: any file that changed during this
#        tick and does not match loop/allowlist_auto.txt is hard-restored to
#        HEAD and logged as VIOLATION; a diff over 20 files or 600 lines
#        trips the circuit breaker and hard-restores EVERYTHING new-this-tick
#        (ESCALATION), regardless of allowlist match. See
#        scripts/ops/loop_guard.py::cmd_enforce.
...
#     itself (its exit code is inspected, not fatal) and the enforce call
#     (best-effort: `|| true`, so a bug in the guard never blocks a
#     legitimate journal append — see scripts/ops/loop_guard.py header for
#     why enforce fails OPEN rather than closed on its own error).
#
# USAGE:
...
#    append that already happened inside the claude run.
