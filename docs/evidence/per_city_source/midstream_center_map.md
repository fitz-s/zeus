...
3. Gate: `_replacement_q_mode_live_eligibility` — only `FUSED_NORMAL_FULL` / `FUSED_NORMAL_PARTIAL` are live-eligible. `BAYES_PRECISION_FUSION_CAPTURE_MISSING`, `FUSED_CENTER_ONLY_NORMAL`, `FUSED_Q_BUILD_FAILED`, `FUSED_NORMAL_BOUNDS_MISSING` all raise `REPLACEMENT_Q_MODE_NOT_LIVE_ELIGIBLE` → no-submit receipt.
