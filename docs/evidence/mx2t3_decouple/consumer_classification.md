...
| `data/executable_forecast_reader.py:296/855 _members / read_executable_forecast_snapshot` | **CARRIER (eligibility) + SHADOW** | Parses `members_json` into an `ExecutableForecastSnapshot`, but its live consumer (`forecast_snapshot_ready` reader-block) reads only `.ok`. Other consumer `entry_forecast_shadow.py` is SHADOW_ONLY. Not a live q source. | KEEP (CARRIER eligibility). |
| `data/entry_forecast_shadow.py:121 read_executable_forecast_snapshot` | **SHADOW** | `evaluate_entry_forecast_shadow` returns `SHADOW_ONLY` absent a separate rollout decision (docstring + body). Readiness telemetry, not live sizing. | KEEP-or-STRIP (shadow). Safe to strip if the entry-forecast cutover telemetry is no longer wanted; no live decision depends on it. |
