...
|-----|-------------|--------|
| [`docs/reference/zeus_math_spec.md`](zeus_math_spec.md) | Complete mathematical and statistical machinery: unit handling, the RAW no-de-bias center law, Ledoit–Wolf covariance shrinkage, precision-weighted Bayesian fusion, Monte Carlo P_raw (now offline evidence baseline), Extended Platt calibration (offline evidence), settlement-exact preimage integration, bootstrap confidence intervals. §0.1 authority notice identifies which sections describe the live path vs the offline evidence baseline. | `[ref]` durable |
