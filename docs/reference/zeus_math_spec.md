...
> **Now implemented (flag-gated, activation pending), do not confuse with §15.1:** an empirical-Bayes *ENS bias correction* applied to forecast member extrema (not Platt-parameter pooling) is implemented — it shrinks the TIGGE structural prior toward live OpenData settled residuals (SNR-gated), with a predictive-error scale term and a 0.5°→0.25° variance transport, applied pre-Monte-Carlo. Gate: `settings.bias_correction_enabled` (default `false`). Code: `src/calibration/ens_bias_model.py`, `src/calibration/ens_error_model.py` (PRs #334/#336). §15.1 below remains deferred and refers to pooling of *Platt* parameters, a different layer.
