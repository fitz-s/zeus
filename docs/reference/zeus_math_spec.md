...
3. Settlement data (Y side: WU / HKO / NOAA; CWA is legacy-retroactive-dispute only, see lore CWA_STATION_IS_LEGACY_DISPUTED_PATH_NOT_DEAD_CODE)
...
> Offline historical calibration only. The live q is the fused-normal-direct settlement integration of §0.1d (`emos.bin_probability_settlement`), not Platt. This Platt math may produce comparison/provenance output only; it must not cap, floor, or veto live replacement q without new authority.
...
### 15.7 Market-cost executable seam

The live path has one market-cost and sizing shape:

```
edge_yes = p_posterior_yes - executable_yes_price
edge_no  = p_posterior_no  - executable_no_price
```

`MarketAnalysis.find_edges` creates a typed `ExecutionPrice` at the edge-scan
boundary. `_size_at_execution_price_boundary` preserves that provenance, applies
the venue fee if necessary, enforces `assert_kelly_safe()`, applies the current
`EffectiveKellyContext` haircut, and calls Kelly. A caller may additionally cap
size with authoritative `max_executable_shares`.

The bootstrap subtracts the same fixed executable price used by the current live
route. There is no optional alternate market-cost sampler, no environment
switch that suppresses selected haircuts, and no second edge regime waiting for
activation. Missing live effective context fails closed.

**Acceptance:** typed price provenance reaches Kelly; no default-off alternate
edge/cost path exists in live modules; replay and cycle runtime use the same
sizing boundary; executable-depth caps remain cumulative when supplied.
