...
**Sizing verdict:** current sizing is strong at the executable-price boundary but still mostly generic. Launch should keep fractional Kelly as the core, but apply **strategy/phase/liquidity/time-to-resolution multipliers** and forbid Kelly on midpoint/current-price semantics. `live_safety_cap_usd = $5` and `smoke_test_portfolio_cap_usd = $5` remain load-bearing until bankroll truth and riskguard stability are fixed.
...
| Sizing                   | Kelly core: `f* × kelly_mult × bankroll`; default dynamic Kelly multiplier 0.25; cluster exposure and portfolio heat throttling; hard caps include `$5` live safety cap and micro caps.                                                              |
...
The draft routes these to `REMAINING_TASKS.md`: data-ingest resilience, riskguard DB/proxy errors, `$150` bankroll fiction, TIGGE historical backfill, PhysicalBounds/ExpiringAssumption follow-ups. `REMAINING_TASKS.md` confirms that riskguard-live flapping is P0, the `$150` bankroll hardcode is structural debt masked by the `$5` cap, and TIGGE backfill is historical-calibration work rather than live-blocking strategy work.
...
| Kelly + caps are sufficient sizing.                     |      §1.3 | Generic Kelly can size all strategies.                                  | `PARTIALLY_CONFIRMED`: typed executable-price Kelly exists; risk limits generic.                                                                     | `MISLEADING`: strategy phase and liquidity change risk.                                                           | `LIVE_BLOCKER`              | Revise                              | Add strategy/phase/liquidity/time multipliers; keep `$5` cap.                                                          |
...
| 10. Report / learning / promotion   | Shadow/live evidence identifies robust cohorts.                                            | Canonical events, strategy_key, metric, phase, fill truth, settlement truth.                        | Executable snapshot hashes and order/fill status.                                                                                                                                                                          | diagnostic/promotion                 | Cohort contamination, paper evidence promoted as live.         | `NEW_LAUNCH_REQUIRED` evidence policy.                                                         |
...
| Execution constraints   | Shadow must still cite executable snapshots; no paper midpoint fantasy. |
...
1. Live safety preconditions
   ...
   └── keep $5 caps until bankroll truth fixed
...
| Bankroll truth               | `REVIEW_REQUIRED` / out-of-scope P0 debt: `$150` hardcode and bankroll truth chain deferred; $5 cap masks risk.   |

### Best sizing design for launch

**Decision:** keep generic Kelly core, but add **strategy-aware caps and multipliers**. Do not replace Kelly. Do not increase the `$5` live cap until riskguard, bankroll truth, and first full entry→fill→settlement lifecycle evidence are clean.
...
| `bankroll`                         | Canonical bankroll; until fixed, capped                           |
| `city/metric exposure`             | Portfolio state                                                   |
| `risk level`                       | Riskguard/current level                                           |

### Per-strategy launch sizing rules

| Strategy               | Multiplier/cap rule                                                                                                      |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `settlement_capture`   | Use Kelly at executable price, but cap at `$5`; require observed-edge confidence; no size if settlement-source mismatch. |
| `center_buy`           | Base dynamic Kelly; cap `$5`; require fresh forecast and CI lower > 0 after FDR; no stale middle live.                   |
...
| `$5` cap invariant                       | Preserve launch cap.                   |
...
| `$150` bankroll hardcode      | Structural bankroll truth                  | Wallet-balance canonical redesign       | Unsafe cap lift                                       | `REMAINING_TASKS.md` §G         |
...
| Tasks                  | Add per-strategy multipliers; hold-to-resolution policy; local killswitch layers; keep $5 caps.                                                                                         |
...
Rollback:
- Disable new multipliers via config defaults and retain $5 cap.

Not-now constraints:
- Do not raise bankroll cap.
...
| Sizing accepted                 | Typed executable price; fee/depth/slippage/tick; strategy/phase multipliers; `$5` cap retained.                                                          |
