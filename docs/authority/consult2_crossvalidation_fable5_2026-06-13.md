...
| Era fitting | EB partial pooling + LRT decision rule | EB partial pooling; LRT = REPORTED EVIDENCE ONLY (pretest estimator has unbounded relative risk near null) | CONVERGENT + refinement (always partial-pool) |
...
| DISPUTED (nee QUARANTINED) | exclude OR confusion-matrix measurement error; weights biased | exclude OR **CAR interval-widening** (Heitjan–Rubin); weights biased for ANY w>0 (estimating-equation proof) | CONVERGENT + refinement (CAR composes with censored likelihood) |
...
| Ledoit–Wolf | identity target hides common drift → factor/constant-corr target | equal-weight pull CONSERVATIVE here; autocorrelation ⇒ LW UNDER-shrinks ⇒ floor ρ≥0.5, cap w∈[0,2/M], drop worst provider explicitly if MSE ratio ≥4 | PARTIAL DIVERGENCE → resolved as floor+evidence check (addendum D4) |
...
1. **Always-partial-pool law**: never use the era LRT as a pool/no-pool switch
   ...
   MLE as n grows. LRT/score test is REPORTED as a evidence check only.
2. **CAR disputed-row treatment** (nee "CAR quarantine treatment"): when the
   per-row settlement ambiguity set A_i
   ...
   unrecoverable or the ambiguity is directional (evidence check: compare clean vs
   disputed covariate distributions + source-disagreement direction).
...
5. **Winner's-curse slope evidence check**: regress realized PnL/contract on shrunk
