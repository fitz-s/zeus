...
- notebooks
- one-off scripts
- temporary backfill read-only inspections
- ad hoc reports
...
### CONST-09
任何 dormant parallel persistence 一律禁止；需要的事实直接进入唯一 canonical owner，不需要的事实不落盘。
...
### 8.6 Dormant parallel persistence
禁止新增：
- 新的 `*_tracker.json`
...
- forbidden dormant parallel persistence
...
6. 这次改动是否新增 dormant parallel persistence？
