...
### CONST-09
任何 shadow 决策面一律禁止 —— operator law 2026-06-13「不准再shadow了」、2026-07-19「shadow…从源头上就不存在」（见 `zeus_current_architecture.md` §10.4 NO SHADOW LANE）。不存在「有删除计划就可暂存」的例外；export-only / compat-cache 非决策文件另见 §8.6。
...
必须通过：
- `LifecyclePhase` enum
- `fold_event(...)` <!-- GAP_NEEDS_OPERATOR (2026-05-17, re-verified 2026-07-21): `def fold_event` still 0 hits in src/. Open — implement, drop requirement, or replace with the actual ledger API. -->
- `apply_transition(...)` <!-- GAP_NEEDS_OPERATOR (2026-05-17, re-verified 2026-07-21): `def apply_transition` now exists but in a different domain (`src/contracts/settlement_outcome.py:106`, SettlementOutcome dispute states — NOT LifecyclePhase); the K0 lifecycle symbol is still absent. Open. -->
...
- 仅允许从 `StrategyKey` enum 取值 <!-- GAP_NEEDS_OPERATOR (2026-05-17, re-verified 2026-07-21): `class StrategyKey` still 0 hits; current design (root AGENTS.md) treats `strategy_key` as a plain string identity, so this enum requirement looks superseded-by-design (dead), not pending — operator to confirm DROP or implement. -->
...
- 禁止其它模块直接 `INSERT INTO position_events` <!-- GAP_NEEDS_OPERATOR (2026-05-17, re-verified 2026-07-21): scope is wider than first flagged — 7 files besides ledger.py hold a direct `INSERT INTO position_events` (reconcile/diff_engine.py, state/chain_reconciliation.py, state/position_duplicate_consolidator.py, execution/exchange_reconcile.py, execution/command_recovery.py, execution/exit_lifecycle.py, events/edli_position_bridge.py). Approved exceptions or §8.5 violations? Unanswered. -->
