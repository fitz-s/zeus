...
| T1.c-followup 5 P9 rewrite + 2 P4 OBSOLETE audit | closed | pending | 1 file modified (tests/test_live_safety_invariants.py, +new `_seed_canonical_entry_baseline` helper) + T1_receipt.json updated. **4 of 5 P9 tests REWRITTEN + PASSING** against post-T4.1b canonical shape: L214 rescues_pending_tracked_fill (canonical baseline + conn), L268 rescue_updates_trade_lifecycle_row (asserts CHAIN_SYNCED event with source_module+payload details), L335 rescue_emits_exactly_one_stage_event (canonical idempotency), L1227 deferred_fill_logs_last_monitor_best_bid (seeds active-phase baseline, EXIT_ORDER_FILLED event with best_bid details). **1 of 5 P9 classified OBSOLETE_PENDING_FEATURE** (L875 day0_transition — no canonical Day0 transition event exists; restoring needs new build_day0_window_entered_canonical_write builder; feature-level slice). **2 P4 OBSOLETE_BY_ARCHITECTURE** (L1536/L1569 — load_portfolio is DB-first with per-row env axiom; JSON contamination-guard scenario doesn't exist in canonical path; no RELOCATE target). **Skip count 8→4 (50% reduction)**; plan `SKIPPED ≤ 1` still unmet (operator decision req'd on OBSOLETE_DELETE for the 3 non-Phase2 skipped tests). Delta (L28): pre-T1.c-followup 4F/47P/8S → post 4F/51P/4S (+4 passes, -4 skips, 0 new failures); planning-lock GREEN (tests/* only); surrogate skipped; con-nyx dispatch continues idle. **T1 family 100% materially closed** with 3 OBSOLETE tests documented for operator disposition | 2026-04-24 |
