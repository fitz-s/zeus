...
| R8 | adapter `_build_live_execution_command_certificates` L3216 → `_require_pre_submit_authority_witness` L3293 → main `_edli_pre_submit_authority_provider_from_book_evidence_conn` L6950 → `_edli_pre_submit_book_from_jit_fetch` L6899 → `get_orderbook_snapshot(token_id)` **[NET]** L6894 | **FRESH best_bid/best_ask** for the selected token | **live CLOB /book HTTP** |
