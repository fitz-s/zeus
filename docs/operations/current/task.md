...
- Follow-up diagnostic branch `fix-live-frontier-diagnostics-20260522` added the missing no-trade attribution handoff: rejected decisions now pass `strategy_key` and `event_source` into `no_trade_events`, so future math/strategy frontier rows do not depend on parsing `reason_detail`.
