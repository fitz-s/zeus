# Active Operations Package Index

authority: false
role: pointer_and_evidence (not architecture law, not runtime truth)

Active package manifest: docs/operations/current/package.yaml
Live control pointer: docs/operations/current_state.md

## Contents

| Subdir | Purpose |
|--------|---------|
| plans/ | Active plan files per subtask slug |
| evidence/ | Investigation and evidence evidence |
| reports/ | Generated non-authority reports |
| closeouts/ | Closeout records (required fields in package.yaml) |
| agent_runtime/ | Agent runtime capsules; non-authority scratch |

Work journal: docs/operations/current/plans/hourly_capital_gains_improvement_loop.md
Plan index: docs/operations/current/plans/INDEX.md
