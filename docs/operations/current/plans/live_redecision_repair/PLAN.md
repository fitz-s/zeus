...
- `src/engine/cycle_runtime.py::execute_monitoring_phase` builds fresh `ExitContext`, refreshes pending-exit quote from current CLOB, calls `Position.evaluate_exit`, and routes true exits through `execute_exit`. `src/strategy/portfolio_rotation.py` and `_emit_portfolio_rotation_shadow_summary` remain shadow-only; switch/rotation is evaluated but not yet live actuation.
...
- `_emit_portfolio_rotation_shadow_summary` computes `best_rotation(...)` using held positions and budget-rejected positive candidates, but records only shadow summary fields. It does not yet submit a sell+replacement-buy chain.
