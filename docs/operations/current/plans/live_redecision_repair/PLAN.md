...
- `src/engine/cycle_runtime.py::execute_monitoring_phase` builds fresh `ExitContext`, refreshes pending-exit quote from current CLOB, calls `Position.evaluate_exit`, and routes true exits through `execute_exit`. `src/strategy/portfolio_rotation.py` is pure math only; the live cycle reports `portfolio_rotation_live_status=disabled:no_live_rotation_executor` until an explicit live sell-then-entry executor owns that handoff.
...
- `_emit_portfolio_rotation_live_status` reports `disabled:no_live_rotation_executor` instead of computing a non-actuating replacement candidate. Switch/rotation must not be surfaced as a live redecision result until a real sell+replacement-buy chain is wired.
