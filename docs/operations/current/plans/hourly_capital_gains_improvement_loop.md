...
### 2026-09-24 current order/exit continuity repair

- Scope: current >24h candidate-to-command attrition, new filled-position belief/exit decisions, and executor post-submit receipt durability. Runtime base live 4d5de5323; role tree 3c539dd96 is tree-equivalent. No new trading policy or manual venue action.
- Verified defect: command 45fe4d3175fe4349 remains SUBMITTING after SDK return; 12:06Z log reports final submission envelope write and REVIEW_REQUIRED append both database-is-locked. Shared BUY/SELL final-envelope helper writes outside the bounded/retried ACK transaction path.
- Implement local persistence isolation using existing write-coordinator and atomic retry semantics; preserve caller-owned transactions, immutable signed identity, side-effect count, final envelope durability and correct recovery on genuine failure. No retry of venue submission.
- Acceptance: real SQLite concurrent writer antibody for both BUY and SELL receipt paths, malformed-envelope and caller-transaction preservation, failed ACK does not erase SDK receipt, no introduced failures relative to base; independent money-path review. Rollback is one source/test commit.
- Parallel read-only scopes: recent filled losses/exit chain and current long-lead auction selection. Each finding must become fix/refute/defer with current causal evidence, not a historic PnL gate or forced order. Deployment/real fills are separate from code verification.
- Current long-lead defect: preflight848508 and 31 sibling attempts stop the whole cut on Atlanta Sep25 LOW old local NO snapshot (04:28Z, clob_no_ask_illiquid) despite a current globally selected maker curve. Implement exact liquidity-only rebind for this selected BUY through mandatory current Gamma/CLOB/raw-book JIT; retain archived/closed/token/source/unknown failures. Validate both YES/NO, ordinary family rejection, JIT book changed/absent, and maker witness requirements. This repairs old quote authority superseding new sealed selection; it does not add a trade quota or claim profit.

- Implementation: exact-global-BUY-only empty-ask rebind, with the same exact liquidity reason at admission and sealed economic binding. Other selection routes retain rejection; current Gamma/CLOB/raw-book JIT replaces the old snapshot before submission. No source, closed-market, missing-row, or identity exception.
- Executor: final SDK envelope commits independently on a clean file-backed connection under bounded canonical write coordination. Review found original 250ms x4 lease insufficient for observed multi-second contention; receipt now uses RECOVERY_CRITICAL 5s x4, holding capped at500ms, while ordinary ACK policy is unchanged. Canonical temporary TRADE/MONITOR1.9s contention fails initial patch and succeeds after repair for BUY and SELL; no SDK retransmission.
- Verification: cache-rejection YES/NO antibodies initially2failed8passed, corrected source10passed; fresh-JIT old-empty-ask pair2passed and current empty-ask/wrong-token negatives4passed. JIT regression set baseline and fixed each106passed2failed with identical failure IDs. Required six engine suites baseline and fixed each93cases39failed with identical IDs. Full qkernel set initial baseline9failed105passed1skipped vs fixed7failed107passed1skipped (two new defect cases resolved); integrated executor/qkernel217passed7same baseline failures1skipped before canonical-wait follow-up. Independent source review found no new critical/important gate defect; segmented relationship coverage, not a live end-to-end fill proof. Planning-lock/map-maintenance/diff checks pass.
- Runtime evidence read-only: loaded4d5de5323 unchanged; user untracked entry_band_pnl plan preserved. No runtime code, live DB, order, account, daemon or risk configuration changed during this slice. These branch fixes are not deployed or realized-profit evidence.
- Remaining disposition: latest late Day0 veto is earlier than some long-lead runner-up, but selection-context vs final-nowcast mismatch is not captured well enough to prove its exact cause; defer rule changes rather than guess. Same-cut rerank ran but receipt write timed out; lock-durability repair addresses one source of contention, not proof all scheduling contention is gone. Amsterdam first18C observation became available after bid was already below executable band; no decision-time advantageous SELL was proved. Future probability/source improvement remains open; do not relabel that case as a solved exit omission.
- Cleanup follows updated one-task worktree instruction; unfinished commits survive on a task branch, test artifacts under/tmp, with no new authority surface. Topology friction none_observed on changed surfaces.
- Final follow-up verification: integrated command-split112passed; independent reviewer closed canonical-contention Important and returned GO. Source/test Ruff76diagnostics in both base and fixed with no introduced diagnostic identities; executor Ruff12 likewise unchanged. Four current raw-book empty-ask/token mismatch cases passed. checked=2026-W39; basis=current branch diffs, temporary SQLite tests and named/tmp logs above; until=recheck-on-use.
