...
- 权威矩阵(B,总控件)→ critic(opus)已复核:**判 REJECT,抓出 decision_certificates authority 反转(会删活钱证书权威)**,4 条修正已应用(见 authority_matrix.md 顶部 CRITIC 修正块 + critic_matrix_review.md)。→ **新增 W2 强制门:writers/readers 重导规则**——任何表分类若依赖 manifest 标签或 consult 断言,必须从实际写者/读者代码重导(B1 反转正是没做这条);此门也管 registry drop 脚本:凡 `legacy_archived` 标签在 drop 前须 writer/reader 复核(decision_log、world decision_certificates 均是活表却被误标)。
