...
Rejected alternative (named decision A): a *field-specific* projection that strips `payload["opportunity_book"]["candidates"]` in place, leaving the payload shape flat. Rejected because the identity/evidence boundary would live in projection code, invisible in the data, and would need editing for every future evidence field — a missing edit silently pulls a evidence into identity. The `_diagnostics` annex **names the boundary in the shape** (Wittgenstein: an unnamed distinction gets violated), makes the projection a trivial one-key strip, and generalizes to E1 evidence, E7 provenance, and forecast_posteriors evidence arrays without new code.
...
- `_diagnostics.opportunity_book.diagnostics_hash` — the **single commitment field**: `sha256(canonical_json(content))`. Content-addresses the evidence for tamper-evidence (retrieve → recompute → compare). Because it lives in the annex, it is **out of identity** — summarizing the book produces a new `content` and a new `diagnostics_hash` **without touching `payload_hash`**.
...
- **W2 — the smallest change that ADDS v2 without touching v1 writes** (this is the "证书 v2 格式" half of W2; the freeze half is already landed at `3d7bf34d1`/`40760942f`):
  ...
  3. `no_submit_receipts.py` — version-aware `_receipt_json`: for v2, add `"schema_version":2` and route the volatile `opportunity_book` evidence out of the hashed payload into a column (mirroring the existing `envelope_json` / C2-columns "store-full-in-column, exclude-from-hash" precedent at `:311-343`); v1 path unchanged.
...
- **W5 — history rewrite, with the hard line:** **never semantically rewrite a v1 preimage.** Concretely:
  - A v1 `ActionableTradeCertificate`'s `opportunity_book` may be **RELOCATED byte-preservingly** (move `payload_json`, or the book substring, to cheaper storage as opaque bytes, still recomputable to the same `payload_hash`) — but may **never be SUMMARIZED**, because v1 `payload_hash = stable_hash(full payload)` binds every evidence byte. Summarization is available **only** to v2 rows (whose identity excludes the annex).
...
- **RECEIPT v2** — pin a `schema_version=2` NO_SUBMIT receipt whose `receipt_json` carries `"schema_version":2` and excludes the volatile `opportunity_book` evidence. Assert mutating the excluded evidence leaves `receipt_hash` invariant, and that `receipt_id` (already three-field) is unchanged from v1.
