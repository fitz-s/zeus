...
control_layer: "exact-SHA deployment proof while preserving the pre-deploy operator control state; this packet does not authorize a new manual entry pause"
...
acceptance:
  ...
  - "Production boots the exact repair SHA without creating a new manual entry pause and produces a current SDK-free rejection receipt."
  ...
  - "Same-family sibling BUYs are decided by correlated portfolio marginal economics; neither global candidate policy nor command persistence imposes a one-position or one-token veto."
---
...
The repair restores defense in depth and adds an independent restart admission check so deleting the same helpers no longer produces a deployable live commit. Existing positions continue through monitoring and exit evaluation. Deployment preserves the operator's pre-deploy control state; this packet does not authorize an autonomous entry pause.
...
- Current runtime evidence shows entries resumed. Deployment must preserve that
  unpaused operator state unless an independent canonical risk or freshness gate
  blocks entry; this correction does not create another manual pause.
- 2026-07-20 correction: the temporary sibling-token exclusivity rule was an
  overconstraint, not operator policy. It is removed; the absolute price band
  remains unchanged and same-family orders return to the correlated portfolio
  capital objective.
