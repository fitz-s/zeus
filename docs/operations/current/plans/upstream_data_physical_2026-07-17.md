...
- Retired-mode migration: operator law forbids; consult agrees cutover-in-one-transaction works.
...
- Fail-closed freshness; no stale-as-fresh. No retired modes. No gate accretion / over-engineering. Minimal machinery.
...
gap exit_quality.md Q2/Q5 measured. Re-verified, not rebuilt (no retired /
...
## Wave-3 status (2026-07-19 late)

- dead-bin exit speed: CLOSED — event-driven exit wake already built+deployed by coworker (ingest day0_extreme_event_committed publish → main.py:4427 socket listener → targeted _exit_monitor_cycle urgent_day0). All 7 historical failures predate it; zero dead-bin misses since. See 94a2f3142 ledger note.
- m5 latch: NO CHANGE WARRANTED — db-locked persistence gap never fired (12d logs); real driver = genuine ws disconnects at negligible cost. capital_utilization.md 55% figure erratum'd; dominant reduce-only driver is kill_switch_armed/heartbeat_lost (58%) → inv-heartbeat-killswitch running.
- high-q overconfidence (docs/evidence/capital_efficiency_2026_07_19/highq_overconfidence.md): CONFIRMED center bias in mu* (HIGH cold z=+0.376, LOW warm z=-0.423, extremum damped toward climatology); sigma nominal; all-Day0 loss (-$37.93); realized edge in 0.80-0.95 band ≈ +1.4pp vs claimed +23.9pp. Legal fix path: settlement_coverage_hierarchy (F1, shrink-only q_exec layer at the Kelly choke point) — currently disarmed AND data-starved. Prerequisites: (1) receipt persistence restored, (2) _hierarchy_observations_all extended to entered-position outcomes (carry strategy_key), (3) invariant tests + walk-forward backtest proof pass, then arm the flag directly in the live Kelly/admission path — no read-only evidence staging step.
- OPEN CROSS-SESSION CONFLICT (operator decision needed): my 342534cbd (receipt persistence + DAY0_LIVE_ADMISSION_REJECTED registration + tautological city_allowlist deletion) was deliberately reverted by the coworker session 17 min later (12c763ce5, no stated reason) — plausibly because the receipt INSERT adds zeus-world.db writer pressure on the live decision path (coworker was landing world-writer contention fixes in the same minutes: f001d38ce, c0da5b97d). Coworker later restored proof_accepted=True themselves (abc70f343) but NOT the receipt wiring / reason registration / allowlist fix. Receipts still 0 rows today. Re-landing needs either coworker coordination or an operator ruling on where the receipt write belongs (off-hot-path queue?). Do NOT silently re-cherry-pick.
- heartbeat/kill-switch driver (docs/evidence/capital_efficiency_2026_07_19/heartbeat_killswitch_driver.md): ALREADY FIXED by coworker — heartbeat LOST formerly tripped FULL kill-switch (froze exits too; 727 log lines/12d of real full-freezes); a42510ca0 (07-18) removed the mapping, d8802c749 (07-19 20:25Z) re-added as reduce-only-only trigger. DB-verified live. Residual flagged: 8s consumer freshness vs 5s writer + 2s HTTP timeout leaves ~3s slack — ordinary CLOB latency still causes entries-only reduce_only blips; widen-window decision needs episode-duration histograms (follow-up).

## Operator ruling 2026-07-19 (remote): re-land reverted work; wave-4 dialectic
...
- Coworker world-writer-pressure hypothesis for the revert: receipt INSERT sits on the live decision path. If contention re-manifests, the fix direction is moving the write off-hot-path (queue/drain), NOT re-reverting the correctness content. Direct check once the next daemon restart picks up 106942322: query `edli_no_submit_receipts` row count immediately post-restart to confirm persistence is live — a single positive read settles it, no standing watch period needed.
...
VERDICT: center bias is a DESIGN CONSEQUENCE of the RAW law, not a defect. The legal correction layer is downstream q-shrink: settlement_coverage_hierarchy (F1) — receipt persistence restored (106942322), position-outcome pool landed (3326e8bc4, STRATEGY_BUCKET/SUPERBUCKET evidence run measured: 0.85-0.90 q_exec 0.710 vs q_raw 0.870), evidence script scripts/shadow_settlement_coverage_hierarchy.py ready (script filename is legacy; the run itself is a one-shot correctness measurement, not a standing observation tier).

OPERATOR DECISION OPEN:
Arm settlement_coverage_hierarchy_enabled directly in the live Kelly/admission path once the walk-forward proof + invariant tests (highq_overconfidence.md §5 item 6) pass — recommended; no staged/read-only evidence tier first.
...
## Retired eradication complete (2026-07-19)

Operator ruling executed at all three layers:
- CODE: 0f3f5a883 — retired-test harness deleted (zero callers proven); F1 hierarchy live layer untouched. Only retired-named file repo-wide eliminated. src/reconcile + src/reduce flagged: inert R2-rebuild scaffolding (own plan, no flag, no callers) — categorically not retired machinery, not swept.
- DOCS: e0c3cf1a8 — wait/retired/fixed-offset proposal shapes rewritten to live-direct decision shapes; RAW-law-reopen option (b) marked DEAD.
- AUTHORITY: 9cde55bce — §10.3 retired lane excised from architecture law (30-day-parallel-data + 7-day-reversible-cutover protocol was the codified source of the forbidden shape); promotion = proof on settled history + operator approval; rollback = git revert.
- DATA: state/ retired artifacts deleted (source_clock_vnext_shadow_20260625.db + sidecars, emos_shadow_ledger.jsonl 292MB, shadow_comparator.json) — zero code references, zero open file handles (lsof clean; the fresh -shm mtime was a stale sidecar, wal empty 0 bytes). All June-era dead runs.

Arm decision for settlement_coverage_hierarchy_enabled now gates ONLY on the q-shrink math consult (in flight) — no retired step, no waiting period.

## Consult verdicts (2026-07-19 late, ~/cgc-answers/)

MERGE-GATE consult (multiwinner five-commit set): CORRECT-BUT-SUBOPTIMAL approach, NO-GO verdict at 94% confidence — arrived AFTER the 19:14 daemon restart already picked the commits up. Live-risk triage at 19:20: epochs_run>1 fired 0 times, event_dead_letters 0 since restart, hierarchy flag ABSENT=off (two of four blockers dormant). Disposition per finding:
- KEEP clean: 46d56436c, be8932f13 (no Day0 mode miscert found), day0_admission allowlist deletion, DAY0_LIVE_ADMISSION_REJECTED registration.
- BLOCKER-1 (LIVE): reactor loop equates venue_call_started with progress, may start epoch K+1 after K's world finalization failed; pagination debits continuation wrongly. Fix: continue only after durable winner finalization; superior realization = durable GLOBAL_AUCTION_CONTINUE event through the scheduler (consult §5.1).
- BLOCKER-2 (LIVE): 106942322 receipts stamped LIVE+proof_accepted are REJECTED by _assert_no_submit_lane_invariant before insertion — the fix defeats itself, may dead-letter. Fix: typed LIVE_PRE_VENUE_ABORT disposition; receipt test must drive the real reactor seam (current test hand-inserts, bypassing the invariant).
- BLOCKER-3+4 (DORMANT, flag off): hierarchy pool reads mutable position_current.p_posterior, timestamp-string comparison, COALESCE('') masking, claim-time fallback = look-ahead channels. Fix before any arm.
Q-SHRINK MATH consult: DO-NOT-ARM at 0.95 — Jeffreys prior itself fine (max 1.6pp at n=30); the disease is (i) the layer sits AFTER selection so receipt-q and capital-q can name different authorities (violates one-authority law), (ii) estimand mismatch (entered+rejected lanes pooled without transport), (iii) fixed-sample intervals used as anytime tests, (iv) super-bucket pooling can wrong-sign a child, (v) no regime epochs. Target: ARM-WITH-CONDITIONS via a CalibratedProbabilityCarrier BEFORE proof construction (7 ranked conditions in answer.md). Interim safer lever it endorses: operator-authorized entry quarantine for the empirically-refuted high-q cohort.

## Consult blockers cleared (2026-07-19 night)

- BLOCKER-1 landed 53ed47af9: multi-winner continuation gated on durable winner finalization (GlobalEpochOutcome.finalized = _finalize_deferred_event_unit success + no dead-letter routing); pagination debit deduped per claimed event; both new adversarial tests verified fail-on-pre-fix.
- BLOCKER-2 landed f99cb9697: typed SUBMIT_LANE_LIVE_PRE_VENUE_ABORT admitted at the no-submit persist boundary ONLY for exact reason allowlist (DAY0_LIVE_ADMISSION_REJECTED + 3 strategy-floor variants) AND venue_call_started=False, re-checked at the invariant; trivialized hand-insert test replaced with real armed-adapter seam drive; plain LIVE/NO_SUBMIT contradiction still unrepresentable. Writer-contention reshape (off-hot-path receipt drain) deliberately split out as a separate slice.
- Both were latent (epochs_run>1 = 0 fires; receipts 0 rows) — now correct BEFORE first exercise. Consult NO-GO's two live blockers thereby cleared; remaining two (hierarchy pool provenance/cutoff) stay dormant behind the off flag with DO-NOT-ARM standing.
- entry-leadtime fix landed e62b72374 (never-priced tier). Orphan stash from the receipt agent's destroyed worktree dropped after byte-identical redo verified; zero agent worktrees remain.

## Cycle-topology + fusion-anchor consult (2026-07-19, ~/cgc-answers/2026-07-19_cycle-topology-fusion-anchor/)

VERDICT (high confidence): (1) 06z/18z exclusion is WRONG for D0/day-ahead — it conflates "cannot cover every configured horizon" with "cannot cover this market"; ECMWF publishes all four cycles through +144h and Zeus's own trigger builds the same 0-144h grid for all four. Promote via TARGET-WINDOW authorization (four predicates: target-window coverage, causal possession, exact manifest completeness, per-source freshness) — NOT a bare flag flip. Nominal issuance interval 12h→6h; expected newest-cycle age −3h, worst-case wait −6h. (2) Independent config defect confirmed: 00z/12z advertises live_max_step 282h while the 150-360h tail is no longer fetched — truthful live max is 144h until restored. (3) anchor_value_c retired as operative type → evidence moment only; authority becomes fused PosteriorLaw over raw per-source curves each with its own source clock; Day0 conditions EACH source curve BEFORE fusion (current ecmwf-only delta on a blended scalar is an inconsistency). (4) 7-phase migration, each provable on settled history, one authority version live at a time, no retired lanes; moment-matched Normal is the sanctioned bridge at Phase 5 before law-level fusion at Phase 6. (5) New-source ranking: direct ICON first (~+2:46-3:20 observed, 3-4h earlier than ECMWF ENS), then direct GFS (~+3:41-3:48), GEFS/GDPS diversity-only, AIFS stays blocked. (6) S1-S14 successor authority clauses drafted in answer.md §7 — includes resolving the replacement_final_form de-bias contradiction in favor of RAW law.
Ranked by freshness-gain/risk: #1 06z/18z target-window auth, #2 manifest-possession events replace schedule timers, #3 source-clock-vector binding, #4 per-source Day0 conditioning, #5 ICON, #6 GFS, #7 law-level fusion.
