...
| 6 | Tier-0 live research mode | UNBLOCKED by operator directive 2026-08-24 ("bp 不是约束,持续稳定盈利才是判据"): full <0.25 band, stake = smallest venue-legal order (flat), no bp cap. IMPLEMENTING (impl-tier0). Aggregate 2% open-cost ceiling + 10% drawdown kill retained (risk_policy.yaml tier0 block). Activation still requires: Item 3 (true p0 certificates) + release checklist. |
