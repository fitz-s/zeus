...
| **INV-PROJ-1** | a projection phase that no event ever produced (a writer bypassing the event log) | replay-diff: every `position_current.phase` must be event-SOURCED — produced by some `position_events.phase_after` (`scripts/dev/replay_position_phase.py --assert-no-diff`) + fixture CI test | ✅ landed (`tests/test_inv_proj1_phase_projection_recomputable.py`). **Correction 2026-06-30:** a first naive cut ("== latest event phase_after") false-positived on 2 positions where observational MONITOR_REFRESHED/REVIEW_REQUIRED events carry a stale phase_after the A5 authority-aware reducer correctly overrides (voided-then-active-monitor; economically_closed-then-quarantine). The corrected event-SOURCING check finds **0 live drift** — every stored phase is event-sourced, so the phase projection is consistent across all live positions. Still the **§7D enabler**: gate each writer-removal on `--assert-no-diff`. **Consult-validated caveat (2026-06-30, [external consult thread]): NECESSARY, not SUFFICIENT** — it proves the stored phase appeared SOMEWHERE in event history, not that it equals the FINAL reducer result, so a stale regression to an earlier-but-once-produced terminal (e.g. `economically_closed` later superseded by `settled`) would PASS. Before using it as the §7D cutover gate, add a companion FINAL-STATE replay invariant: fold the ordered event stream through the A5 **authority** reducer and assert derived-final-phase == `position_current.phase` (the authority-ranked fold — NOT the recency "latest event" fold already rejected). |
...
Consult round-3 ([external consult thread], web-grounded in Polymarket/UMA docs) — third independent convergence.
