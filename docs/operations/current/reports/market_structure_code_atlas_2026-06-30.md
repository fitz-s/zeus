...
| **INV-PROJ-1** | a materialized projection not recomputable from source facts / a projection writer bypassing the projector | replay-diff: `position_current.phase` must equal the latest non-null `position_events.phase_after` (`scripts/dev/replay_position_phase.py --assert-no-diff`) + fixture CI test | ✅ landed (`tests/test_inv_proj1_phase_projection_recomputable.py`). **Live run found 2 real drift cases** — `economically_closed` vs event `quarantined`, and `voided` vs event `active` (the phantom-void class §7A) — confirming the multi-writer drift is real, not theoretical. This is the **§7D enabler**: gate each writer-removal on `--assert-no-diff`. |
