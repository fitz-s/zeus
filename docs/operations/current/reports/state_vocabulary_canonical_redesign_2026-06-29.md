...
- **New `src/state/canonical_projections.py`** — IMPLEMENTED: `derive_position_phase()`, `derive_exit_progress()`, `derive_order_result_status()`, `derive_recovery_class()`, exposure predicates (`counts_as_active_exposure`/`is_closed_exposure`/`is_optimistic_exposure`/`weighted_lot_exposure_micro`), `is_open_order_fact`/`OPEN_ORDER_FACT_STATES`. DEFERRED (part of the family-dedup rewrite, NOT yet in this module): `family_dedup_blocks_entry()`, `exposure_micro_for_governor()`. REUSE `position_truth.py`'s existing typed facts (`LocalIntent`/`VenueOrderFact`/`VenueTradeFact`/`VenuePositionFact`, `FillAuthority`/`CausalityStatus`) — do NOT create parallel `CommandFact`/`OrderFact`/`TradeFact` (that would re-introduce redundancy).
