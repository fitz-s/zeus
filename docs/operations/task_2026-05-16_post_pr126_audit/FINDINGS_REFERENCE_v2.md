...
| **F36** | **Settlement live tables empty since 2026-05-07 (post `_archived_2026_05_11` migration) — root cause of empty bridge output even when scheduled** | **SEV-1** | **NEW (Run #10)** | src/execution/harvester.py + src/ingest/harvester_truth_writer.py + state/zeus-world.db | Run #10 | Run #10 |
...
| **F38** | **calibration_pairs_v2 = 0 rows → entire OOS-staleness antibody is no-op** | **SEV-1** | **NEW (Run #10) INVESTIGATE-FURTHER** | scripts/rebuild_calibration_pairs_v2.py + writer path TBD | Run #10 | Run #10 |
