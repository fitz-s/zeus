...
2. **P3's `--v-next-shadow` (shipped in the `topology_v_next_phase2_shadow` packet) has reached 95% AGREE over the 14-day shadow window.** The AGREE rate is defined in MIGRATION_PATH §Phase 2: ≥ 95% of shadow comparisons over a rolling 7-day window must show AGREE (old-engine and v_next agree on admission outcome). Note: P1 defers the `--v-next-shadow` wire-up diff to P3 (P1 rev 1.2 §3); P2.0 therefore gates on P3's shadow metric, not P1's. This threshold must be met BEFORE the P2.a window opens, because the AGREE signal is the primary instrument for detecting regressions. If P2 ships before 95% AGREE, any companion-required MISSING_COMPANION emissions will conflate with residual divergence signals, making both metrics uninterpretable.
...
4. **The two shadow windows MUST NOT overlap.** P3's shadow window and P2's shadow window are sequential, not concurrent. Overlapping them would conflate P3 divergence signals with P2 companion signals; the AGREE rate metrics would conflate; false-positive attribution would be impossible. Enforcement: the P2 PR cannot be opened until the P1 promotion commit exists on main.
