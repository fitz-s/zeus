...
| F3 | Shadow candidates write `phase0_backfill` and fabricate `gamma_explicit` anchor | completed | Added `write_shadow_decision_event()`; all six shadow enter candidates use `source='shadow_decision'`; missing anchor becomes `unknown_legacy`; schema v25 admits the new source/anchor. |
...
- Post-main-merge related pytest: 200 passed / 1 deselected across executor, command split, Phase 4 T2/T3/T4 candidate provenance/no-trade, shoulder cluster/ledger, cluster exposure, regime correlation store, and strategy profile registry tests. The one deselected test imports `cycle_runner` and requires local `sklearn`, which is not installed in this workspace.
- Phase 6 merge smoke: 46 passed across evidence-tier, shadow experiment registry, regret decomposer, live readiness tribunal, and strategy-profile evidence-tier tests.
