...
## Changelog v4 → v5 (2026-05-16, post-G2-round-3-critic revision + main re-merge)

G2 round-3 opus critic on v4: 4 PASS + 2 SEV-1 + 2 SEV-2 + minor. The two SEV-1s were (a) **checkout drift**: v4 "honest disclosure" cited `src/main.py:L965-988` from a grep run on `/Users/leofitz/.openclaw/workspace-venus/zeus/` (main checkout, +20 commits ahead) — audit worktree at HEAD `f7a46c4f5c` actually had spans at L908-931. (b) **--force over-permission**: v4 widened --force to all-except-CONFIRMED, but REDEEM_SUBMITTED has an in-flight adapter call window — operator running --force during SUBMITTED creates a double-redeem race that crystallizes the moment PR-I.5 wires real adapter.

v5 fixes:

- **NEW-P15a (SEV-1) — checkout drift RESOLVED via re-merge**: merge commit `c952acd299` brings audit worktree to align with main (+20 commits since `ba128c8e65`). Re-grep in post-merge audit worktree confirms scheduler.add_job spans at L965, 973, 978, 984 (harvester), 985-986 (heartbeat), 988 — matching SCAFFOLD's claimed range. ZERO conflicts during re-merge; settlement_commands.py CHECK + Enum intact at L31-34 + L71-78. **Plus durable fix**: §K.8 now mandates `executor's G3 first action: re-grep scheduler.add_job in audit worktree before edit`, with the literal command, to prevent future checkout drift.
- **P2-v4 (SEV-1) — --force on SUBMITTED EXCLUDED**: v5 §K.4 restricts --force allowed source states to `{REDEEM_INTENT_CREATED, REDEEM_RETRYING, REDEEM_TX_HASHED, REDEEM_OPERATOR_REQUIRED, REDEEM_REVIEW_REQUIRED, REDEEM_FAILED}`. **REDEEM_SUBMITTED explicitly excluded** because it indicates in-flight adapter call (operator pasting tx_hash during this window = double-redeem hazard once adapter is real). To rescue a wedged SUBMITTED row, operator must wait for the existing retry/timeout path OR use a separate admin step (deferred to PR-I.5 follow-up).
- **NEW-P12-v4a (SEV-2 → SEV-1 if shipped as-written) — migration FK violation handling**: v5 §K.8 migration row inserts step 8.5 `PRAGMA foreign_key_check BEFORE COMMIT, abort transaction on any violation rows`. Per-DB atomic mode (each DB in its own transaction; partial-failure across DBs flagged). Daemon-stop prerequisite documented in migration script docstring; CLI rejects run if `state/zeus-live.db` is locked (file-lock check).
- **NEW-P11-v4 (SEV-2) — antibody test ast parse**: v5 §K.6 antibody test spec switches from `inspect.getsource` regex to `ast.parse + walk` for the state-literal-in-transition check; covers both `"REDEEM_OPERATOR_REQUIRED"` string literals and `SettlementState.REDEEM_OPERATOR_REQUIRED` enum attribute access. More robust to future refactors.
- **§K.4 `submitted_at` explicit (minor)**: v5 §K.4 body now states `submitted_at = CLI_INVOCATION_TS (operator-recorded; semantic distinct from adapter-submission time)` instead of `submitted_at=?` placeholder. Audit event payload retains the distinction via `actor=operator` field.

**Meta-pattern (acknowledged)**: round 3 of SCAFFOLD critic caught checkout drift — exactly the kind of error per `feedback_audit_of_audit_antibody_recursive.md`. v4's "honest disclosure" was itself a wrong-checkout error; v5 fixes by re-merging the base AND making executor's first-grep the authority going forward. The 4-round pattern (v1 → v2 operator pivot → v3 round-1 fix → v4 round-2 fix → v5 round-3 fix) shows each layer caught a different class of issue; converging on PASS.

**Author of v5**: main session (executor role, opus model). Post-merge HEAD `c952acd299`; SCAFFOLD content commit on top.
...
3. Atomic transition: UPDATE settlement_commands SET state='REDEEM_TX_HASHED', tx_hash=?, submitted_at=CLI_INVOCATION_TS
   WHERE command_id=? AND state='REDEEM_OPERATOR_REQUIRED'. Assert cursor.rowcount == 1. **`submitted_at` semantic (v5)**: CLI invocation timestamp (operator-recorded UI claim time), NOT adapter-submission time. The semantic distinction is preserved via the audit event payload (`actor='operator'`); future PR-I.5 auto-submission path will write `submitted_at = adapter_call_ts` instead.
...
FORCE MODE (--force, recovery use; v5 P2-v4-tightening — exclude SUBMITTED in-flight window):
- Allowed source states (v5): **{REDEEM_INTENT_CREATED, REDEEM_RETRYING, REDEEM_TX_HASHED, REDEEM_OPERATOR_REQUIRED, REDEEM_REVIEW_REQUIRED, REDEEM_FAILED}**.
- **EXCLUDED**: REDEEM_SUBMITTED (in-flight adapter call — operator pasting tx_hash during this window crystallizes double-redeem hazard once PR-I.5 wires real adapter). EXCLUDED: REDEEM_CONFIRMED (true terminal, on-chain success).
- To rescue a wedged SUBMITTED row: operator must wait for existing retry/timeout path OR use a separate `--admin-transition-submitted-to-review-required` step (deferred to PR-I.5 follow-up; documented but not implemented in PR-I).
- Same atomicity (single UPDATE), WHERE clause: `WHERE command_id=? AND state IN ('REDEEM_INTENT_CREATED','REDEEM_RETRYING','REDEEM_TX_HASHED','REDEEM_OPERATOR_REQUIRED','REDEEM_REVIEW_REQUIRED','REDEEM_FAILED')`. Assert cursor.rowcount == 1; otherwise reject + audit.
...
- **NEW v4 + v5 NEW-P11 critic fix (ast parse instead of regex)**: for each entry with `terminal_states_with_operator_action`, each listed state must have: (a) a transition INTO it from src/ — verified by **`ast.parse` walk** (not `inspect.getsource` regex) over candidate transition call-sites in `src/execution/settlement_commands.py`; the walk asserts the target state literal OR enum attribute access (e.g. both `"REDEEM_OPERATOR_REQUIRED"` string and `SettlementState.REDEEM_OPERATOR_REQUIRED` Attribute access) appears as a constant argument to one of the `_transition`/`_atomic_transition` call expressions. AST coverage is robust against regex false-negatives from formatting changes. (b) `max_age_hours` field set and positive integer, (c) `operator_runbook` field resolves to an existing file (anchor not verified — too brittle), (d) `cli_invocation` field is a non-empty string.
...
| `scripts/migrations/202605_add_redeem_operator_required_state.py` | NEW (v3 P7 + v4 NEW-P12 + v5 NEW-P12-v4a critic fixes) | SQLite CHECK-constraint migration. **PREREQUISITE (v5 critic fix)**: daemon must be stopped; CLI rejects run if `state/zeus-live.db` file-lock is held (`fcntl.flock` check on the file descriptor); operator runs `launchctl stop com.zeus.live` first. **Per-DB atomic mode (v5)**: each target DB runs in its own transaction; if one DB succeeds and the next fails, the script reports per-DB outcome and exits non-zero — operator decides recovery. Steps per DB: (1) **Idempotency check first**: `SELECT sql FROM sqlite_master WHERE name='settlement_commands' AND sql LIKE '%REDEEM_OPERATOR_REQUIRED%'` — if non-empty, log "migration already applied, no-op" and skip to next DB. (2) `PRAGMA foreign_keys=OFF` (required because `settlement_command_events.command_id REFERENCES settlement_commands(command_id)` — see `settlement_commands.py:59`; rebuild would otherwise break FK). Note: PRAGMA foreign_keys must be set OUTSIDE transaction per SQLite docs. (3) `BEGIN IMMEDIATE TRANSACTION`. (4) `CREATE TABLE settlement_commands_new` with full v5 CHECK (includes REDEEM_OPERATOR_REQUIRED). (5) `INSERT INTO settlement_commands_new SELECT * FROM settlement_commands`. (6) `DROP TABLE settlement_commands`. (7) `ALTER TABLE settlement_commands_new RENAME TO settlement_commands`. (8) Recreate the 3 indexes (idx_state, idx_condition, ux_active_condition_asset per `settlement_commands.py:49-55`). **(8.5) `PRAGMA foreign_key_check` BEFORE COMMIT — if any rows returned, abort transaction (`ROLLBACK`), log violation list, exit non-zero** (v5 NEW-P12-v4a fix; prevents corrupt-DB post-commit). (9) `COMMIT`. (10) `PRAGMA foreign_keys=ON`. (11) Repeat for next DB if applicable. **Dry-run mode (v5)**: `--dry-run` flag prints the planned actions per DB without executing; operator verifies before commit. **Target DBs (dual)**: runs against both `state/zeus_trades.db` and `state/zeus-live.db`; CLI arg `--db <path>` for per-DB invocation; default mode runs both sequentially. **Test**: `tests/test_migration_redeem_operator_required.py` covers: (a) row preserved, (b) new state accepted, (c) FK valid post-migration, (d) re-running migration is no-op, (e) artificially-induced FK violation triggers ROLLBACK + non-zero exit, (f) `--dry-run` mode does not modify DB. | Schema migration on live DB |
...
| `src/main.py` | **v5 RESOLVED — post-merge audit worktree (commit `c952acd299`+)**: re-grep confirmed scheduler.add_job spans at L965, 973, 978, 984 (harvester), 985-986 (heartbeat), 988 (last existing). Insert 3 new `scheduler.add_job` blocks + `_assert_cascade_liveness_contract(scheduler)` call **after L988**. **v5 executor MANDATE — re-grep BEFORE edit**: G3 first action is `grep -n 'scheduler.add_job\|_harvester_cycle\|_write_heartbeat' src/main.py` in the audit worktree (post-merge HEAD). If output differs from the L965-988 spans cited here, audit worktree has drifted further — STOP G3 and re-merge main before continuing. This grep-gate per `feedback_grep_gate_before_contract_lock.md` is the durable fix for the v4 checkout-drift defect that round-3 critic caught. | Scheduler registration |
