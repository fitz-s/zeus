...
valid/first-valid time, fetch time, and effective available-at time. Accepted
decisions persist those fields into `epistemic_context_json.forecast_context`.
This closes the scoped entry-side "missing source/timing/hash evidence can
continue" path. It does not choose the canonical live snapshot table, activate
TIGGE/direct ECMWF, promote Open-Meteo, or implement the full cross-path
`CausalTimestampSet`.
...
Phase 1J status for DSA-10/DSA-18 (2026-04-29): replay snapshot-only fallback
is now explicit opt-in for every replay mode. `run_replay()` no longer treats
`mode != "audit"` as permission to set `allow_snapshot_only_reference=True`;
counterfactual and walk-forward lanes must pass the same market-events
preflight unless the caller deliberately requests diagnostic fallback.
`shadow_signals`, `ensemble_snapshots.available_at`, and forecast-row fallback
remain available only through that explicit diagnostic path, and replay output
continues to stamp `promotion_authority=False`. This slice does not rename or
migrate `shadow_signals`; that table-name cleanup remains future DSA-10 work.
...
available-at time. Accepted decisions persist that evidence in
`epistemic_context_json.forecast_context`. Open-Meteo monitor/diagnostic
fallback still cannot satisfy entry authority; no issue time is fabricated.
...
- One table name still simultaneously means pre-trading shadow, live telemetry,
  and replay fallback.
- Phase 1J reduces the authority risk by preventing replay mode names from
  implicitly enabling diagnostic fallback, but the table name itself remains
  semantic debt.
...
2. Freeze `shadow_signals` as legacy diagnostic input or migrate it to a clearly named `legacy_shadow_signal_diagnostic` table/view.
3. Require replay fallback outputs to carry `diagnostic_non_promotion` authority and prevent promotion/economics consumers from reading them — partially implemented by explicit fallback opt-in plus `promotion_authority=False`.
...
- Phase 0B closes that path: live authority must now be explicit at the
  `get_mode()` seam.
...
4. Require scripts to pass explicit mode or use read-only DB paths — enforced
   by the `get_mode()` seam for callers that rely on mode state paths.
...
cannot reach executable entry with missing forecast evidence. This reduces the
legacy/v2 split's audit blind spot, but DSA-13 remains open for the canonical
live snapshot table decision.
...
- `src/engine/cycle_runtime.py:1301-1318` creates entry intents without passing those snapshot facts.
...
- `src/execution/fill_tracker.py:24` treats `MATCHED` as filled on the legacy polling path.
...
issue/valid/fetch/available timing and records decision-time status in accepted
decision context. The broader observation/market/command/fill/settlement/replay
`CausalTimestampSet` is still a separate open design slice.
