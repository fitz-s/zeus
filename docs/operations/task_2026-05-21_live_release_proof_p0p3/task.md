...
| P2-1 | §7 P2 docs/scaffold drift, §16 step 8 | Refresh authority headers after runtime verification | FIXED | 2026-05-21 §7 P2 docs/scaffold drift / §16 step 8 | Updated stale `gamma_explicit` authority text in `architecture/db_table_ownership.yaml` and `src/strategy/market_phase.py`; runtime/report trust remains `unknown_legacy` by default unless explicit source evidence exists |
| P2-2 | §7 topology registries | Runtime-to-registry assertion in CI | FIXED | 2026-05-21 §7 topology registries | Added `tests/test_live_release_registry_runtime_assertions.py`; money-path CI now runs it and watches `architecture/db_table_ownership.yaml` |
| P2-3 | §7 monitor nowcast fail-soft | Counter/alert on persistent nowcast write failures | FIXED | 2026-05-21 §7 monitor nowcast fail-soft | Added nowcast write failure counter plus persistent-failure log after consecutive failures; test asserts counter and alert |
| P2-4 | §7 hardcoded strategy/cluster maps | Move risk metadata to reviewed config with tests | FIXED | 2026-05-21 §7 hardcoded strategy/cluster maps | Evaluator uses reviewed `city.cluster` for regime exposure instead of `city.name`; registry test pins this relationship |
| P3-1 | §7 sed-revert antibodies | Convert key antibodies to relationship/replay tests | FIXED | 2026-05-21 §7 sed-revert antibodies / §14 | P0-4 lifecycle replay, P1 family reducer relationship tests, P1 Kelly dynamic matrix, P1 anchor report-trust tests, and P2 registry/runtime tests replace key sed-revert-only coverage with cross-module assertions |
| P3-2 | §7 pre-existing failures | Track failure IDs centrally | FIXED | 2026-05-21 §7 pre-existing failures | Added `architecture/pre_existing_failure_registry.yaml` and `tests/test_pre_existing_failure_registry.py`; entries require IDs, owner, evidence, review deadline, and forbid permanent ignored status |
