...
- Pre-Phase 0B evidence: earlier `src/config.py` revisions defined only
  ...
  `ZEUS_MODE` to `live`.
- Current Phase 0B status: `src/config.py:45-57` ignores `ZEUS_MODE` and
  returns the single live runtime; `tests/test_k5_slice_l.py::TestGetMode`
  locks explicit live, missing-env, and retired non-live-env behavior.
- `src/main.py:602-609` now consumes that live-only helper; daemon startup is
  no longer gated by an environment string.
...
- `src/engine/replay.py` uses the physical `shadow_signals` table only as
  legacy diagnostic storage; replay source output now reports
  `legacy_shadow_signal_diagnostic` with `storage_source=shadow_signals`.
- `src/engine/replay.py` can use `shadow_signals` storage as a fallback only
  when `allow_snapshot_only_reference` is true.
...
- Current Phase 1J follow-up status: `shadow_signals` fallback no longer
  appears as a replay decision-reference source. The physical storage name is
  retained as `storage_source`, while the semantic source is
  `legacy_shadow_signal_diagnostic` and the validation set records
...
- Pre-repair `src/execution/fill_tracker.py:24` treated `FILLED` and
  `MATCHED` as fill statuses on the legacy polling path. This was the concrete
  F06 symptom; the 2026-04-30 follow-up now treats both as non-final
  observations and requires `CONFIRMED` for success terminality.
...
- Phase 0B: retired `ZEUS_MODE` as runtime authority; `get_mode()` is the
  single live runtime contract.
...
- `src/config.py`: removes environment-mode authority; missing or invalid
  `ZEUS_MODE` no longer changes or blocks the live runtime.
- `tests/conftest.py`: may set `ZEUS_MODE=live` as a compatibility helper, but
  config-backed modules no longer depend on that value for runtime authority.
...
- `tests/test_k5_slice_l.py`: proves explicit live, missing env, and retired
  non-live env values all resolve to the live runtime contract.
...
Phase 0B recertification run (superseded by 2026-04-30 live-constant
recertification):

- historical `python3 scripts/topology_doctor.py --navigation --task "DSA-12 get_mode explicit ZEUS_MODE ..."` run
  -> historical evidence for the old explicit-env plan. Current law is
  `phase 0b zeus mode retirement`: `get_mode()` ignores `ZEUS_MODE` and returns
  live.
- `pytest -q -p no:cacheprovider tests/test_k5_slice_l.py::TestGetMode tests/test_config.py::test_get_mode_is_live_constant_not_env_authority tests/test_config.py::test_settings_mode_key_is_legacy_optional tests/test_digest_profile_matching.py::test_dsa12_zeus_mode_retirement_routes_to_phase0b_profile tests/test_digest_profiles_equivalence.py`
  -> 10 passed.
...
- Existing replay provenance tests keep ensemble snapshot, forecast-row, and
  `shadow_signals` storage fallback available under explicit opt-in.
- 2026-04-30 follow-up: replay output now reports `shadow_signals` storage as
  `legacy_shadow_signal_diagnostic`, carries `storage_source=shadow_signals`,
  and stamps `authority_scope:diagnostic_non_promotion` in diagnostic fallback
  validations.
- Replay output records `authority_scope=diagnostic_non_promotion` and
  `promotion_authority=False`; this slice does not rename or migrate
  `shadow_signals` and does not claim economics authority.
...
- `pytest -q -p no:cacheprovider tests/test_run_replay_cli.py::test_counterfactual_replay_does_not_auto_enable_snapshot_only_reference tests/test_run_replay_cli.py::test_run_replay_allows_snapshot_only_reference_opt_in tests/test_run_replay_cli.py::test_run_replay_snapshot_only_can_fallback_to_forecast_rows tests/test_replay_time_provenance.py::test_replay_context_snapshot_only_fallback_is_opt_in tests/test_replay_time_provenance.py::test_replay_context_can_fallback_to_shadow_signal_reference tests/test_digest_profile_matching.py::test_dsa10_dsa18_snapshot_only_fallback_routes_to_phase1j_profile tests/test_digest_profiles_equivalence.py`
  -> 10 passed.
- 2026-04-30 follow-up focused test:
  `pytest -q -p no:cacheprovider tests/test_replay_time_provenance.py::test_replay_context_can_fallback_to_shadow_signal_reference tests/test_run_replay_cli.py::test_run_replay_shadow_signal_fallback_uses_legacy_diagnostic_source tests/test_run_replay_cli.py::test_run_replay_snapshot_only_can_fallback_to_forecast_rows`
  -> 3 passed.
- 2026-04-30 follow-up replay suite:
  `pytest -q -p no:cacheprovider tests/test_replay_time_provenance.py tests/test_run_replay_cli.py`
  -> 25 passed.
- 2026-04-30 follow-up compile check:
  `python3 -m py_compile src/engine/replay.py tests/test_run_replay_cli.py tests/test_replay_time_provenance.py`
  -> pass.
- 2026-04-30 follow-up topology:
  `python3 scripts/topology_doctor.py --navigation --task "Phase 1J DSA-10 DSA-18 replay snapshot-only fallback explicit opt-in; shadow_signals diagnostic replay fallback authority; no DB mutation; no live venue; no Paris source routing" --files src/engine/replay.py tests/test_replay_time_provenance.py tests/test_run_replay_cli.py docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> navigation ok; profile `phase 1j replay snapshot-only fallback explicit
  opt-in`; admission status `admitted`.
- 2026-04-30 follow-up profile regression:
  `pytest -q -p no:cacheprovider tests/test_digest_profile_matching.py::test_dsa10_dsa18_snapshot_only_fallback_routes_to_phase1j_profile tests/test_digest_profiles_equivalence.py`
  -> 5 passed.
- 2026-04-30 follow-up semantic lint:
  `python3 scripts/semantic_linter.py --check src/engine/replay.py tests/test_run_replay_cli.py tests/test_replay_time_provenance.py docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> pass.
- 2026-04-30 follow-up planning lock:
  `python3 scripts/topology_doctor.py --planning-lock --changed-files docs/operations/AGENTS.md src/engine/replay.py tests/test_replay_time_provenance.py tests/test_run_replay_cli.py docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md --plan-evidence docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> topology check ok.
- 2026-04-30 follow-up map maintenance:
  `python3 scripts/topology_doctor.py --map-maintenance --map-maintenance-mode closeout --changed-files docs/operations/AGENTS.md src/engine/replay.py tests/test_replay_time_provenance.py tests/test_run_replay_cli.py docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> topology check ok.
- 2026-04-30 follow-up source-label sanity grep:
  `rg -n '"source": "shadow_signals"|decision_reference_source_counts.*shadow_signals' src/engine/replay.py tests/test_replay_time_provenance.py tests/test_run_replay_cli.py || true`
  -> no matches.
- 2026-04-30 follow-up whitespace checks:
  `git diff --check -- src/engine/replay.py tests/test_replay_time_provenance.py tests/test_run_replay_cli.py`
  plus trailing-whitespace scan over the untracked packet docs -> pass.
- Independent code-reviewer subagent review after the 2026-04-30 follow-up
  reported no scoped code findings. The reviewer confirmed explicit opt-in,
  `legacy_shadow_signal_diagnostic` source labeling,
  `storage_source=shadow_signals` traceability, diagnostic non-promotion
  authority stamping, and no schema/DB/source-routing/live-venue or economics
  promotion scope creep. Residual review caveat: auxiliary MCP diagnostics
  (`lsp_diagnostics` / `ast_grep_search`) failed with `Transport closed`, so
  the review was evidence-based rather than MCP-complete.
...
- Superseded by the 2026-04-30 follow-up below: exit-side
  `exit_lifecycle.py` was later tightened to the same `CONFIRMED`-only
  finality contract.
...
## Phase 5D Full Market-Price Linkage Readiness Guard - 2026-04-30

Scope:

- Tighten DSA-19 readiness so `market_price_history` rows cannot be interpreted
  as promotion-grade market-price evidence unless they explicitly prove full
  decision-time CLOB linkage.
- Preserve no-go boundaries: no schema migration, no DB mutation, no WebSocket
  capture, no live venue side effects, no CLOB cutover, no PnL computation,
  and no economics tombstone weakening.

Implementation:

- `src/backtest/economics.py::check_economics_readiness()` now checks
  `market_price_history` for a full-linkage contract, not just row presence.
- Required full-linkage evidence is explicit:
  `market_price_linkage="full"`, a CLOB source
  (`CLOB_WS_MARKET`, `CLOB_BEST_BID_ASK`, or `CLOB_ORDERBOOK`), valid
  `best_bid <= best_ask` in `[0, 1]`, and a non-empty
  `raw_orderbook_hash`.
- Existing scanner/Gamma price-only rows remain useful forward substrate, but
  they now produce `market_price_history_lacks_full_linkage_contract` and
  cannot clear the economics readiness surface.
- This deliberately does not change `market_price_history` DDL. A future
  WebSocket/orderbook capture packet must add or project the required
  full-linkage fields before this readiness blocker can disappear.

Verification:

- `python3 scripts/topology_doctor.py --navigation --task "Phase 5D promotion-grade economics readiness full market price linkage guard DSA-19 economics tombstone bid ask orderbook fee tick neg-risk parity no live venue side effects no production DB mutation no schema migration" --files src/backtest/economics.py tests/test_backtest_skill_economics.py docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> navigation ok under `phase 5 promotion grade economics readiness
  implementation`.
- `python3 -m py_compile src/backtest/economics.py tests/test_backtest_skill_economics.py`
  -> pass.
- `pytest -q -p no:cacheprovider tests/test_backtest_skill_economics.py`
  -> 13 passed.
- `pytest -q -p no:cacheprovider tests/test_backtest_skill_economics.py tests/test_market_scanner_provenance.py::TestForwardMarketSubstrateProducer::test_forward_substrate_writes_verified_scanner_rows_without_unblocking_economics tests/test_schema_v2_gate_a.py::TestApplyV2SchemaSmoke::test_apply_v2_schema_creates_market_price_history tests/test_schema_v2_gate_a.py::TestApplyV2SchemaSmoke::test_forward_market_substrate_writer_works_after_apply_v2_schema tests/test_runtime_guards.py::test_discovery_phase_writes_verified_forward_market_substrate_before_evaluator`
  -> 17 passed, 2 subtests passed.
- An earlier verification command used stale node names for two existing tests
  and failed collection with `no tests ran`; `rg`/`--collect-only` was used to
  identify the current node ids, and the corrected command above passed.
- `python3 scripts/topology_doctor.py --planning-lock --changed-files docs/operations/AGENTS.md src/backtest/economics.py tests/test_backtest_skill_economics.py docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md --plan-evidence docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> topology check ok.
- `python3 scripts/topology_doctor.py --map-maintenance --map-maintenance-mode closeout --changed-files docs/operations/AGENTS.md src/backtest/economics.py tests/test_backtest_skill_economics.py docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> topology check ok. The first closeout run without `docs/operations/AGENTS.md`
  failed because this packet directory is still an added operations surface and
  the existing operations registry companion must be included in the closeout
  set.
- `python3 scripts/semantic_linter.py --check ...` over the scoped source,
  test, and packet docs -> pass.
- `git diff --check -- ...` over the scoped source, test, and packet docs
  -> pass.
- Independent code-reviewer -> APPROVE. The review verified that price-only
  rows now fail closed, full-linkage-shaped rows are accepted only up to the
  still-active `economics_engine_not_implemented` tombstone, and the change has
  no DDL, writes, commits, default DB opens, production DB access, or live venue
  side effects.

Residual blockers:

- DSA-19 remains open. Promotion-grade economics still requires real forward
  market/venue/probability/selection/settlement substrate, confirmed
...
- The entry evidence gate requires explicit `source_id`, `model`,
  ...
  `fetch_time`, and explicit `available_at`.
- Review remediation upgraded the timing check from presence-only to causality:
  `decision_time` must be supplied and parseable, and
  `issue_time <= available_at <= fetch_time <= decision_time` must hold. It
  intentionally does not require
  `issue_time <= first_valid_time`, because Day0 and some provider payloads can
  expose a payload first-valid hour that is earlier than the decision-time
  subset being used.
...
- Phase 1K now has a dedicated narrow topology profile,
  `phase 1K live decision snapshot causality gate`, instead of riding on the
  broader Phase 1 forecast-source profile. The profile admits only the
  evaluator, focused tests, digest artifacts, and packet evidence; it does not
  admit source-registry, config, replay, scripts, execution, venue, or state
  files.

Verification:

- `pytest -q -p no:cacheprovider tests/test_center_buy_repair.py tests/test_runtime_guards.py tests/test_fdr.py tests/test_decision_evidence_runtime_invocation.py tests/test_digest_profile_matching.py::test_phase1k_live_decision_snapshot_causality_routes_to_snapshot_causality_profile tests/test_digest_profile_matching.py::test_phase1k_review_remediation_wording_routes_to_snapshot_causality_profile`
  -> 171 passed.
- `pytest -q -p no:cacheprovider tests/test_ensemble_client.py tests/test_forecast_source_registry.py`
  -> 20 passed.
- `pytest -q -p no:cacheprovider tests/test_digest_profile_matching.py`
  -> 122 passed.
- `python3 -m py_compile src/engine/evaluator.py tests/test_center_buy_repair.py tests/test_runtime_guards.py tests/test_fdr.py tests/test_decision_evidence_runtime_invocation.py tests/test_digest_profile_matching.py architecture/digest_profiles.py`
  -> pass.
- `python3 scripts/digest_profiles_export.py --check` -> pass.
- `python3 scripts/topology_doctor.py --navigation --task "Phase 1K review-remediation entry forecast evidence causality gate snapshot causality profile missing available_at issue fetch available knowability-before-decision no source routing no TIGGE activation no production DB mutation no live venue side effects" ...`
  -> navigation ok under `phase 1K live decision snapshot causality gate`.
- `python3 scripts/topology_doctor.py --planning-lock --changed-files ... --plan-evidence docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> topology check ok.
- `python3 scripts/topology_doctor.py --map-maintenance --map-maintenance-mode closeout --changed-files ...`
  -> topology check ok.
- `python3 scripts/semantic_linter.py --check ...` over scoped source, tests,
  topology, generated digest, and packet docs -> pass.
- `git diff --check -- ...` over scoped Phase 1K files -> pass.
...
- After adding the affected evaluator consumer fixtures to the dedicated Phase
  1K topology profile, exact-scope navigation for Phase 1K admitted
  ...
  `phase 1K live decision snapshot causality gate`.
- `python3 scripts/topology_doctor.py --map-maintenance --map-maintenance-mode closeout --changed-files ...`
  -> topology check ok.
- `python3 scripts/topology_doctor.py --planning-lock --changed-files ... --plan-evidence docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> topology check ok.
...
Phase 1K review-remediation addendum:

- Implemented timestamp ordering in `src/engine/evaluator.py`:
  executable-entry forecast evidence now requires parseable `decision_time`,
  parseable issue/valid/fetch/available timestamps, explicit `available_at`,
  and `issue_time <= available_at <= fetch_time <= decision_time`.
- Added regressions proving missing `available_at`, future
  `available_at`/`fetch_time`, and `available_at` before issue time all block
  before snapshot persistence.
- Updated direct evaluator regressions to supply `decision_time`; this aligns
  test fixtures with live-cycle causality requirements instead of preserving an
  accepted no-decision-time direct-call path.
- Split Phase 1K routing into the dedicated narrow topology profile and updated
  the digest regressions to assert source/data files are not admitted and that
  real review-remediation wording routes to the profile.
- Verification after remediation:
  `pytest -q -p no:cacheprovider tests/test_center_buy_repair.py tests/test_runtime_guards.py tests/test_fdr.py tests/test_decision_evidence_runtime_invocation.py tests/test_digest_profile_matching.py::test_phase1k_live_decision_snapshot_causality_routes_to_snapshot_causality_profile tests/test_digest_profile_matching.py::test_phase1k_review_remediation_wording_routes_to_snapshot_causality_profile tests/test_digest_profile_matching.py::test_phase1k_remediation_rereview_wording_keeps_forbidden_files_out_of_scope`
  -> 172 passed.
  `python3 -m py_compile src/engine/evaluator.py tests/test_center_buy_repair.py tests/test_runtime_guards.py tests/test_fdr.py tests/test_decision_evidence_runtime_invocation.py tests/test_digest_profile_matching.py architecture/digest_profiles.py`
  -> pass.
  `python3 scripts/digest_profiles_export.py --check` -> pass.
  ...
  `pytest -q -p no:cacheprovider tests/test_digest_profile_matching.py`
  -> 123 passed.
  `python3 scripts/topology_doctor.py --navigation --task "Zeus Phase 1K remediation re-review entry forecast evidence causality gate snapshot causality profile explicit available_at knowability-before-decision no source routing no TIGGE activation no production DB mutation no live venue side effects" ...`
  -> navigation ok under `phase 1K live decision snapshot causality gate`.
  `python3 scripts/topology_doctor.py --navigation --task "Zeus Phase 1K remediation re-review entry forecast evidence causality gate snapshot causality profile explicit available_at knowability-before-decision no source routing no TIGGE activation no production DB mutation no live venue side effects" --files src/engine/evaluator.py src/data/forecast_source_registry.py config/settings.json src/engine/replay.py tests/test_center_buy_repair.py`
  -> navigation ok false / admission blocked with forbidden hits for
  `src/data/forecast_source_registry.py`, `config/settings.json`, and
  ...
  -> topology check ok.
  `python3 scripts/semantic_linter.py --check ...` over the scoped source,
  tests, topology, generated digest, and packet docs -> pass.
  `git diff --check -- ...` over the same scoped files -> pass.

Post-interruption recovery verification:

- A final verifier subagent attempt was interrupted by network transport after
  the second code-review remediation. The two prior independent reviewer passes
  had already identified and driven the remaining changes; local deterministic
  gates were rerun after resumption.
- `python3 scripts/digest_profiles_export.py --check` -> pass.
- `python3 -m py_compile src/engine/evaluator.py tests/test_center_buy_repair.py tests/test_runtime_guards.py tests/test_fdr.py tests/test_decision_evidence_runtime_invocation.py tests/test_digest_profile_matching.py architecture/digest_profiles.py`
  -> pass.
- `pytest -q -p no:cacheprovider tests/test_center_buy_repair.py tests/test_runtime_guards.py tests/test_fdr.py tests/test_decision_evidence_runtime_invocation.py tests/test_digest_profile_matching.py::test_phase1k_live_decision_snapshot_causality_routes_to_snapshot_causality_profile tests/test_digest_profile_matching.py::test_phase1k_review_remediation_wording_routes_to_snapshot_causality_profile tests/test_digest_profile_matching.py::test_phase1k_remediation_rereview_wording_keeps_forbidden_files_out_of_scope`
  -> 172 passed.
- Normal Phase 1K topology navigation for the exact scoped files -> admitted
  under `phase 1K live decision snapshot causality gate`.
- Forbidden-probe topology navigation containing `src/data/forecast_source_registry.py`,
  `config/settings.json`, and `src/engine/replay.py` -> fail-closed
  `admission_status=blocked` with those three forbidden hits.
- `python3 scripts/topology_doctor.py --planning-lock --changed-files ... --plan-evidence docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> topology check ok.
...
- User constraint clarified that `MATCHED`/`MINED` are not finality and
  `CONFIRMED` is the success terminal state. A broader audit found the current
  code already kept `MATCHED`/`MINED` optimistic, but still treated venue/order
  `FILLED` alongside `CONFIRMED` in several fill-finality seams.
- `src/execution/exit_lifecycle.py`: `FILL_STATUSES` is now
  `{"CONFIRMED"}` only. `FILLED` no longer triggers exit economic close.
- `src/execution/fill_tracker.py`: `FILL_STATUSES` is now `{"CONFIRMED"}`;
  `MATCHED` and `FILLED` are explicit optimistic observations. Stale dependency
  constants containing `MATCHED` or `FILLED` are subtracted before the finality
  branch.
- `src/engine/cycle_runner.py`: `PENDING_FILL_STATUSES` is now
  `{"CONFIRMED"}` so runtime deps no longer advertise `FILLED` as finality.
- `src/execution/command_recovery.py`: unknown-side-effect recovery now maps
  venue `FILLED` to `PARTIAL_FILL_OBSERVED`, not `FILL_CONFIRMED`; only
  `CONFIRMED` advances command fill finality.
- `src/state/db.py`: pending-exit fill telemetry emits
  `EXIT_FILL_CONFIRMED` only for `CONFIRMED`; `FILLED` records a checked
  observation.
- `tests/test_live_safety_invariants.py`: added a regression proving a pending
  exit with venue status `FILLED` remains `pending_exit` and does not
  economically close; updated the deferred-fill closeout test to use
  `CONFIRMED`.
- `tests/test_command_recovery.py`: added `FILLED` to the nonconfirmed
  unknown-side-effect statuses and narrowed the success test to `CONFIRMED`.
- `tests/test_cross_module_relationships.py`: now asserts both finality modules
  expose exactly `{"CONFIRMED"}`.

Verification:

- `python3 -m py_compile scripts/verify_truth_surfaces.py scripts/venus_sensing_report.py src/engine/cycle_runner.py src/execution/exit_lifecycle.py src/execution/fill_tracker.py src/execution/command_recovery.py src/state/db.py tests/test_command_recovery.py tests/test_cross_module_relationships.py tests/test_live_safety_invariants.py`
  -> pass.
- `pytest -q -p no:cacheprovider tests/test_command_recovery.py tests/test_cross_module_relationships.py::test_exit_lifecycle_fill_statuses_match_fill_tracker tests/test_live_safety_invariants.py::test_deferred_confirmed_fill_logs_last_monitor_best_bid tests/test_live_safety_invariants.py::test_pending_exit_filled_status_does_not_economically_close tests/test_live_safety_invariants.py::test_pending_exit_matched_status_does_not_economically_close tests/test_live_safety_invariants.py::test_matched_without_filled_size_does_not_materialize_entry tests/test_live_safety_invariants.py::test_legacy_polling_matched_maps_numeric_live_runtime_id_to_optimistic_lot tests/test_exchange_reconcile.py::test_nonconfirmed_full_size_trade_is_optimistic_exposure_not_fill_finality tests/test_exchange_reconcile.py::test_confirmed_full_size_trade_is_required_for_fill_finality tests/test_runtime_guards.py::test_monitor_refresh_has_no_production_paper_mode_branch`
  -> 25 passed.
- `pytest -q -p no:cacheprovider tests/test_venue_command_repo.py::TestAppendEventStateTransitionIsGrammarChecked::test_acked_to_filled 'tests/test_venue_command_repo.py::TestAppendEventStateTransitionIsGrammarChecked::test_illegal_transition_raises_value_error[FILLED-FILL_CONFIRMED-setup_events19]' tests/test_exit_safety.py::test_mutex_released_on_cancel_confirmed_or_filled_or_expired tests/test_user_channel_ingest.py::test_matched_event_does_not_final_close_lot tests/test_user_channel_ingest.py::test_confirmed_event_finalizes_trade_and_permits_canonical_pnl tests/test_user_channel_ingest.py::test_exit_sell_confirmed_trade_does_not_mint_positive_exposure_lot tests/test_user_channel_ingest.py::test_ws_path_emits_equivalent_command_events_when_enabled`
  -> 7 passed.
- `python3 scripts/topology_doctor.py --freshness-metadata --changed-files scripts/verify_truth_surfaces.py scripts/venus_sensing_report.py tests/test_command_recovery.py tests/test_cross_module_relationships.py tests/test_live_safety_invariants.py`
  -> topology check ok.
- `python3 scripts/topology_doctor.py --planning-lock --changed-files scripts/verify_truth_surfaces.py scripts/venus_sensing_report.py src/execution/exit_lifecycle.py src/execution/fill_tracker.py src/execution/command_recovery.py src/engine/cycle_runner.py src/state/db.py tests/test_command_recovery.py tests/test_cross_module_relationships.py tests/test_live_safety_invariants.py --plan-evidence docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> topology check ok.
- `git diff --check` -> pass.

Residual risk:

- `CommandState.FILLED` remains the internal terminal command state produced by
  the `FILL_CONFIRMED` event in `src/state/venue_command_repo.py`. This is not
  venue `FILLED` finality; the event boundary is now `CONFIRMED`-only.
- Entry polling can still materialize local optimistic exposure from a
  `FILLED` order observation when filled size is explicit. That is not
  calibration/learning finality and remains separated from confirmed trade
  facts and confirmed lots.

Phase 3 partial-trade-id follow-up (2026-04-30):

- `src/execution/fill_tracker.py`: partial/PARTIALLY_MATCHED polling payloads
  with a real venue trade id now map to optimistic `MATCHED` trade facts when
  the payload omits a separate `trade_status` field. Payloads without real
  trade id still record only order facts and command observations; no synthetic
  trade truth is created.
- `tests/test_command_recovery.py::test_partial_polling_with_trade_id_projects_optimistic_lot`
  proves a linkable partial payload appends `venue_order_facts.state='PARTIALLY_MATCHED'`,
  `venue_trade_facts.state='MATCHED'`, and
  `position_lots.state='OPTIMISTIC_EXPOSURE'`.

Verification:

- `python3 scripts/topology_doctor.py --navigation --task "R3 U2 fill finality closure legacy fill polling MATCHED CONFIRMED partial fill materialization venue_trade_facts position_lots" --files src/execution/fill_tracker.py tests/test_command_recovery.py docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> navigation ok; profile `r3 fill finality ledger implementation`.
- Earlier DSA-15 probes using non-canonical wording routed to `generic` and
  failed closed; the exact R3/U2 fill-finality wording above is the admitted
  profile path.
- `python3 -m py_compile src/execution/fill_tracker.py tests/test_command_recovery.py`
  -> pass.
- `pytest -q -p no:cacheprovider tests/test_command_recovery.py::test_partial_polling_with_trade_id_projects_optimistic_lot 'tests/test_command_recovery.py::TestRecoveryResolutionTable::test_unknown_side_effect_nonconfirmed_status_stays_partial_not_fill_finality[MATCHED]' tests/test_command_recovery.py::TestRecoveryResolutionTable::test_unknown_side_effect_confirmed_reaches_fill_finality`
  -> 3 passed.
- Initial pytest attempt without quoting the parameterized `[MATCHED]` node
  failed at the zsh selector layer with `no matches found`; rerun with quoted
  selector passed as above.

Phase 3 partial-trade-id closeout verification (2026-04-30):

- Strengthened the regression to read the actual `venue_trade_facts.trade_fact_id`
  before checking `position_lots.source_trade_fact_id`; the test no longer
  relies on a fixed row id in the temp DB.
- `python3 -m py_compile src/execution/fill_tracker.py tests/test_command_recovery.py`
  -> pass.
- `pytest -q -p no:cacheprovider tests/test_command_recovery.py`
  -> 16 passed.
- `pytest -q -p no:cacheprovider tests/test_command_recovery.py tests/test_live_safety_invariants.py::test_matched_without_filled_size_does_not_materialize_entry tests/test_live_safety_invariants.py::test_legacy_polling_matched_maps_numeric_live_runtime_id_to_optimistic_lot tests/test_live_safety_invariants.py::test_partial_remainder_cancel_preserves_filled_exposure`
  -> 19 passed.
- `python3 scripts/semantic_linter.py --check src/execution/fill_tracker.py tests/test_command_recovery.py docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> pass.
- `python3 scripts/topology_doctor.py --planning-lock --changed-files src/execution/fill_tracker.py tests/test_command_recovery.py docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md --plan-evidence docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> topology check ok.
- `python3 scripts/topology_doctor.py --map-maintenance --map-maintenance-mode closeout --changed-files docs/operations/AGENTS.md src/execution/fill_tracker.py tests/test_command_recovery.py docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> topology check ok.
- `git diff --check -- src/execution/fill_tracker.py tests/test_command_recovery.py`
  plus a packet-doc trailing-whitespace scan over `findings.md`, `evidence.md`,
  and `simplification_plan.md` -> pass.

Independent review and parameterization follow-up (2026-04-30):

- Code-reviewer verdict for the DSA-15 follow-up: no findings. The reviewer
  confirmed `CONFIRMED` remains the only final fill status, stale
  `MATCHED`/`FILLED`/partial dependency constants cannot bypass the explicit
  optimistic branch, partial payloads with real trade ids map to `MATCHED`
  trade facts and `OPTIMISTIC_EXPOSURE` lots, and no-trade-id payloads do not
  synthesize trade truth.
- Reviewer residual gaps: LSP/AST MCP tools were unavailable (`Transport
  closed`), local `pyright`/`mypy` were unavailable, and the new regression
  directly covered only `PARTIALLY_MATCHED` before follow-up.
- Follow-up action: parameterized
  `tests/test_command_recovery.py::test_partial_polling_with_trade_id_projects_optimistic_lot`
  over `PARTIAL`, `PARTIALLY_MATCHED`, and `PARTIALLY_FILLED`, removing the
  direct-coverage gap while keeping the same production behavior.
- `python3 -m py_compile src/execution/fill_tracker.py tests/test_command_recovery.py`
  -> pass.
- `pytest -q -p no:cacheprovider tests/test_command_recovery.py`
  -> 18 passed.
...
- `python3 scripts/topology_doctor.py --map-maintenance --map-maintenance-mode closeout --changed-files docs/operations/AGENTS.md src/execution/fill_tracker.py tests/test_command_recovery.py docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> topology check ok.
- `git diff --check -- src/execution/fill_tracker.py tests/test_command_recovery.py`
  plus packet-doc trailing-whitespace scan -> pass.

Post-merge DSA-14/DSA-16/DSA-19 boundary verification (2026-04-30):

- DSA-14 entry-only topology probe:
  `python3 scripts/topology_doctor.py --navigation --task "r3 U1 executable market snapshot v2 implementation DSA-14 market identity entry path only no adapter edit no live venue side effects no production DB mutation no source routing" --files src/contracts/executable_market_snapshot_v2.py src/data/market_scanner.py src/engine/cycle_runtime.py src/execution/executor.py tests/test_executable_market_snapshot_v2.py tests/test_market_scanner_provenance.py tests/test_executor_command_split.py tests/test_v2_adapter.py docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> navigation ok; profile `r3 executable market snapshot v2 implementation`.
- DSA-14 adapter-expansion probe including `src/venue/polymarket_v2_adapter.py`
  under the same U1 profile -> navigation ok false; profile
  `r3 executable market snapshot v2 implementation`; admission
  `scope_expansion_required`; out-of-scope file `src/venue/polymarket_v2_adapter.py`.
  Conclusion: entry snapshot identity is in-scope and verified; replacing the
  remaining adapter compatibility shim is a separate venue/live-envelope design
  slice, not a safe implicit follow-up.
- DSA-14 focused verification:
  `pytest -q -p no:cacheprovider tests/test_executable_market_snapshot_v2.py tests/test_market_scanner_provenance.py tests/test_executor_command_split.py::TestLiveOrderCommandSplit::test_entry_submit_requested_persists_execution_capability_proof tests/test_v2_adapter.py::test_polymarket_client_bound_envelope_bypasses_legacy_compat_submit`
  -> 67 passed.
- DSA-16 Phase 2E focused verification:
  `pytest -q -p no:cacheprovider tests/test_exit_safety.py::test_cancel_requested_persists_execution_capability_before_cancel_callable tests/test_settlement_commands.py::test_redeem_submitted_persists_execution_capability_before_adapter_contact tests/test_digest_profile_matching.py::test_phase2e_cancel_redeem_capability_routes_to_dedicated_profile`
  -> 3 passed.
- DSA-16 source-degradation/freshness wording probe routed back to the broad
  Phase 2C execution capability profile and T4 live-side-effect gate rather
  than a narrow residual profile. Current `ExecutionIntent` carries executable
  snapshot identity but not a unified source-degradation/freshness capability
  component. Conclusion: adding those cross-lane components requires a new
  profile/design slice rather than an unplanned edit to executor intent/schema.
- DSA-19 next-producer probe:
  `python3 scripts/topology_doctor.py --navigation --task "Phase 5C upstream forward substrate production DSA-19 next missing producer readiness no live venue side effects no production DB mutation no source routing" --files src/state/db.py src/engine/cycle_runtime.py tests/test_pnl_flow_and_audit.py tests/test_backtest_skill_economics.py docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> navigation ok false; profile `phase 5 forward substrate producer implementation`;
  admission `scope_expansion_required` for the requested economics test files.
  Existing Phase 5D keeps promotion-grade economics tombstoned until real full
  CLOB price linkage, confirmed venue facts/lots, probability/selection parity,
  settlement/outcome facts, and an economics engine exist.

Paris known-gaps source-boundary companion update (2026-04-30):

- `docs/operations/known_gaps.md` now records the source-boundary evidence
  already captured in this packet: observed Paris HIGH contracts resolve on
  `LFPG` through 2026-04-18 and on `LFPB` from 2026-04-19 onward; Paris LOW
  slugs were not observable for 2026-04-15..2026-04-22, and the first
  observable Paris LOW event was 2026-04-23 resolving on `LFPB`.
- This is documentation-only evidence recording. It does not edit
  `config/cities.json`, update `current_source_validity.md`, mutate production
  DBs, run backfills, rebuild calibration, unquarantine Paris candidates, or
  authorize live venue actions.
- `python3 scripts/topology_doctor.py --navigation --task "Paris source-boundary evidence recording LFPG LFPB resolutionSource no config edit no production DB mutation no source routing" --files docs/operations/known_gaps.md docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> navigation ok; profile `source boundary evidence recording`.
- `pytest -q -p no:cacheprovider tests/test_digest_profile_matching.py::test_paris_source_boundary_evidence_routes_to_docs_only_profile`
  -> 1 passed.
- `python3 scripts/topology_doctor.py --planning-lock --changed-files docs/operations/known_gaps.md docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md --plan-evidence docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> topology check ok.
- `python3 scripts/topology_doctor.py --map-maintenance --map-maintenance-mode closeout --changed-files docs/operations/AGENTS.md docs/operations/known_gaps.md docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  -> topology check ok.
- `git diff --check -- docs/operations/known_gaps.md docs/operations/task_2026-04-29_design_simplification_audit/findings.md docs/operations/task_2026-04-29_design_simplification_audit/evidence.md docs/operations/task_2026-04-29_design_simplification_audit/simplification_plan.md`
  plus packet-doc trailing-whitespace scan -> pass.
