...
- Add `path: "docs/operations/archive/cold"` zone entry (cold archive target). **D7 RESOLVED (Batch-C)**: `expected_empty: true` field already present in `topology.yaml` (added W4). Parser wired in `scripts/topology_doctor_docs_checks.py::check_expected_empty_zones` (~35 LOC) + dispatched via `scripts/topology_doctor.py::_check_expected_empty_zones` + called from `scripts/topology_doctor_registry_checks.py::run_docs`. Absent path treated as empty (no error). `.gitkeep` fallback rejected — parser is cleaner. Tests in `tests/test_topology_doctor_docs_checks.py` cover happy case (gitkeep-only), absent path, and stray file failure.
