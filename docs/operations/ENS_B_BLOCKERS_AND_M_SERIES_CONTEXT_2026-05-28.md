...
| D6 | LOW producer fit on staging at sd3 | ⏸️ BLOCKED by D5.5 lock (chain script 42047 waiting) | First attempt PID 15100 wrote 8 LOW rows (partial) then `OperationalError: locked`. Retry PID 20425 also blocked. Chain script `chain_low_mc.sh` (PID 42047) waiting on PID 7167 exit; on clean exit it auto-launches LOW MC rebuild on the same staging DB. Heartbeat 06:41 CT: PID 7167 still alive 67min, WAL 267MB, log byte-delta ~+10k/30min (per-city UNIT-REJECTs: Amsterdam → Beijing → Dallas + others). |
