...
| D6 | LOW producer fit on staging at sd3 | ✅ FELL 2026-05-28 ~05:46 CT (8 rows under sd3, partial during HIGH MC; bulk lock released only after HIGH MC done). |
| D5.5 / D11(HIGH) | External MC HIGH rebuild | ✅ FELL **2026-05-28 09:21 CT** | PID 7167 exited clean after 3:43h. **pair-batch manifest recorded** (SD4/BL-C antibody fired): `pair_batch_id=ddfbfabb37638aa9b0874151639c78ef5854055e6b7d06a28f61f5b6e4236b36 gate=deabf8f64bde27b7 n_rows=71 n_sigs=71`. Per-city pair counts logged. 14.8M ft_v1/HIGH pairs written to staging. Bulk lock released. |
| D11(LOW) | LOW MC rebuild | ✅ FELL **2026-05-28 11:06 CT** | PID 23682 exited clean after 1:44h. **pair-batch manifest recorded** (SD4/BL-C antibody fired ✓): `pair_batch_id=f5abace86e9d74c85b3219c1af3699d8631281718e31609ebe4329859953c681 gate=deabf8f64bde27b7 n_rows=8 n_sigs=8`. Stats: 161,385 snapshots scanned → 3,325 processed (155,536 fail-open skipped due to LOW data sparsity + `low_window_not_target_full:AMBIGUOUS_CROSSES_LOCAL_DAY_BOUNDARY` contract-evidence rejects). **329,280 ft_v1/low pairs written** across 8 cities (Hong Kong 18,156 · London 63,648 · Miami 55,936 · NYC 34,868 · Paris 90,066 · Seoul 37,740 · Shanghai 16,728 · Tokyo 12,138). LOW production scope = 8 cities only under sd3 strict cutoff. |
| D7 | Re-verify D2-D5 for LOW rows | ✅ FELL **2026-05-28 12:55 CT** | 8 LOW STAGING rows under sd3 gate, all MAM only, all `coverage_months={4,5}` ⊆ MAM `{3,4,5}`; 0 NULL/empty coverage; 0 SH-cities (correct — sd3 strict-cutoff excluded BA/Cape Town/Sao Paulo/Wellington from LOW set). Cities: Hong Kong, London, Miami, NYC, Paris, Seoul, Shanghai, Tokyo. D5-equivalent (row audit) already covered by D12 (combined 87-row audit). |
| D8 | Emit fresh `ROW_ACTION_MANIFEST_2026-05-28.csv` under sd3 | ✅ FELL **2026-05-28 14:44 CT** | `audit_error_model_row_reproducibility.py --world-db /private/tmp/scratch_ens_fit.db --forecasts-db /private/tmp/scratch_ens_fit.db --family full_transport_v1 --out docs/operations/ROW_ACTION_MANIFEST_2026-05-28.csv` → 29,854 bytes, 87 rows + header. Schema is per-row reproducibility audit (status/reason/recomputed_*), not legacy A/B/C/D/E action-cohort csv. Closes BL-A / HB8 the operator hard blocker. |
| D9 | Verify manifest classifications sane | ✅ FELL **2026-05-28 14:44 CT** (effectively rolled into D12) | `awk` breakdown matches D12: 72 REPRODUCIBLE, 13 INSUFFICIENT_PRIOR (sd2-tagged identity rows), 2 COVERAGE_MISLABELED (Paris DJF cov=(2,), Sao Paulo DJF cov=(2,)), 0 NON_REPRODUCIBLE, 0 FIT_ERROR. Note: -28 schema dropped legacy `action,how` A/B/C/D/E columns — that cohort assignment moved out of audit. Status-distribution sanity = D12 + D9 unified. |
| D10 | **GATE — all D1-D9 GREEN** | ✅ ALL CLEAR **2026-05-28 14:44 CT** | D2-D7 GREEN, D11 fell external (HIGH+LOW), D12 covers 87-row audit, D8/D9 manifest emitted. Operator rule satisfied; rebuild prerequisites complete. |
...
| D13 | Wait #359 merge | 🔵 PARTIAL ✅ **OPEN 2026-05-28 11:27 CT** | PR #359 opened: title `refactor(state)-PR3/3: B1-B7 generation-naming eradication + F5 trade_decisions demoted + sub-branch merges`. **Naming collision**: #359's "B1-B7" = generation-naming eradication, NOT my pre-MC hard-blockers B1-B7. **Stacked PR**: base = #357 (PR2), not main. Full merge sequence: #355 (PR1) → #357 (PR2) → #359 (PR3) → main. 6,438 additions / 11,543 deletions = big rename diff as operator warned. Mergeable: yes. CI: 5 SUCCESS, 1 FAILURE (topology-context required, no_override), 1 IN_PROGRESS. M1 rebase still BLOCKED on full stack reaching main. |
