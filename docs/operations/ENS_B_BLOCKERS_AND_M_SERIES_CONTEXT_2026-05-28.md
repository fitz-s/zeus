...
| D11(LOW) | LOW MC rebuild | 🔵 IN FLIGHT | PID **23682** launched 2026-05-28 09:22 CT (manual launch — chain script grep was strict, didn't match `pair-batch manifest recorded` final line). Same script + flags, `--temperature-metric low`. Monitor armed. Will rebuild ft_v1/low pairs (~687K existing legacy rows will be replaced). |
