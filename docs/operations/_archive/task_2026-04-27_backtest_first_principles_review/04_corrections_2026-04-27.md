...
|---|---|---|---|
| U1 | Zeus's `crosscheck_members: 31` corresponds to NOAA GEFS (30 perturbed + 1 control) | **VERIFIED** | Trace at HEAD `5bd9be8`: `evaluator.py:1192` calls `validate_ensemble(gfs_result, expected_members=ensemble_crosscheck_member_count())`. `gfs_result` is populated at the same call site by `fetch_ensemble(city, ..., model="gfs025")` in `src/data/ensemble_client.py`, which hits `https://ensemble-api.open-meteo.com/v1/ensemble`. Since this is the **ensemble** endpoint (not the deterministic forecast endpoint), `gfs025` resolves to NOAA's GEFS at 0.25° resolution. GEFS canonical shape = 30 perturbed + 1 control = 31 members. Crosscheck path is the GEFS→evaluator GFS-vs-ECMWF agreement check during entry. |
...
| U4 | `orderbook-subgraph` indexes orderbook snapshots at arbitrary timestamps (vs only events) | **RESOLVED — EVENTS ONLY, NO SNAPSHOTS** | Verbatim from `raw.githubusercontent.com/Polymarket/polymarket-subgraph/main/orderbook-subgraph/schema.graphql` 2026-04-28: 5 entities only — `OrderFilledEvent` (executed trades), `OrdersMatchedEvent`, `MarketData`, `Orderbook` (aggregated trade counts/volumes), `OrdersMatchedGlobal`. **No entity captures bid/ask price snapshots at arbitrary timestamps.** Implication: Subgraph is sufficient for realized-fill reconstruction (what trades printed, at what price, for which market) but insufficient for counterfactual ECONOMICS ("what would Zeus have done at decision time?" requires the ask price at that timestamp, which is not retained). Historical orderbook snapshots require forward-only WebSocket capture OR a third-party paid archive (Tardis / Kaiko / Dune-style). |
