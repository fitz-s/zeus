...
before reading any file. **Authority carve-out:** this file is
authoritative for path → tier mapping (the path table only). Tier
*definitions*, severity model, large-PR rule, evidence rule, and
reporting template live in `code_review.md` (canonical doctrine) and
`REVIEW.md` (compressed mirror). Doctrine conflicts resolve to
`code_review.md`; path-mapping conflicts resolve to this file.
...
docs/reference/security_false_positives.md — gitleaks false-positive registry (review only when changed)
dated lock-state files (e.g. `LIVE_TRADING_LOCKED_*.md`) — review only when changed
.zeus/**                            — runtime cache
...
- New top-level directories under `src/` should be added here when
  introduced. Drift between this file and the actual repo is itself an
  Important finding (concerning a Tier 3 surface).
