...
**〔2026-07-09 v3 落地,取代下述 v2 执行体;方法论不变〕** 执行体改 `codex exec --sandbox workspace-write`(Seatbelt OS 沙箱 = 姿态 A 裁决所要的真边界,实测 src/`.git`/网络/仓外全 DENY);单 tick(L2 折入证据驱动 grading);`loop/INTERVAL` 操作员节奏旋钮(1-6h 不 hardcode);查询 escrow(tick 写 SQL 探针、wrapper 只读代跑 = 预登记机械化);wrapper `commit-auto` 代 commit(tick 无 .git 写);allowlist 收窄 `loop/ docs/ tests/`(architecture 出列 → 本节 DESIGN-RISK 落案,且 loop commit 不再可能触发 deployment-freshness pause)。设计原稿:`docs/operations/current/plans/allday_improvement_loop_v3_codex_2026-07-09.md`。下述 v2 建造史与四轮对抗定理保留为 lineage —— 其结论("同 UID 无 in-tree 技巧可闭合,真边界=OS 沙箱")正是 v3 换内核的依据。
