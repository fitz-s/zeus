...
- NEXT:①操作员两步(F2 迁移 → supersession backfill --apply);②LX-2R-b read-model 物化 + 生产 replay + cent-equivalence 门(gated on ①);③LX-3R fenced activation 设计包(dual-capable 分支/混合事务拆分/selector 迁移/DB 列防火墙)。
