...
- 2026-07-13 wave LX-0R+1R 并行派发(7 packet,sonnet×worktree,文件疆界互斥):A contracts/epoch · B payout observer · C fill sync · D balance head+token registry+allocation policy · E attribution+receipts(**EDLI bridge 未含 — 见 wave-1 审计 C1**) · F identity supersession · G ghost-sell P&L 止血。
- 2026-07-13 wave-1 落地:A 7f00e92a0 · B fa689e8f7 · C 3b08139f1 · D 6acb37116 · E 23248f72e · G 19976478d;F f2c50ebd5 → **操作员 revert af902a8e4(理由未注明)**。集成验证:wave 12 测试面 194 绿、exchange_reconcile/venue_command_repo 93 失败 byte-identical pre-existing、boot ALL PASS。
- 2026-07-13 双审判决(外部 GPT-5.6 `consult_answers/local_ledger_excision_wave1_review_2026-07-13.txt` + 本地 opus 对抗 `..._wave1_local_verifier_2026-07-13.md`):**双 NO-GO(对完整 LX-2R 单元);双准 reducer 实现并行开工**。共享 blocker:C1 EDLI canonical fact bridge 未交付(edli_position_bridge.py:1002 仍是活经济学写者;round-2 明文"before LX-3R, not at LX-3R");C2 LX-F revert 后 identity-supersession 事实缺席(reducer 历史 backfill 硬前置)。本地审证据性反驳外部三条:T4 watermark(get_trades 全量扫描不转发 since,wall-clock watermark 是完备性证明非续传游标 — Attack A 不复开);T1 CHECK(UNKNOWN-带值缺口真实但写者恒 NULL、零消费者 — 收紧非 blocker);E receipts(archive-before-overwrite 保语料 — 软于法条但非毁灭)。外部独有正确项:G 的 fill_price 字段名撒谎 + UNKNOWN→0 cost basis;0R capability 过广告 ACTIVE_NEW(inert 但须在 LX-2R 收窄)。
- 2026-07-13 修复波 wave-1.5 派发(4 packet):E2 EDLI fact bridge · F2 identity supersession 重做(带 CHECK 迁移+grammar 复验) · H 小修集(T1 CHECK 收紧/manifest line-insensitive/G 字段更名+UNKNOWN 保 NULL/0R capability 收窄) · I foreign-fill observation lane。LX-2R reducer 合成实现随后并行,cent-equivalence 里程碑 gated on E2+F2。
- 2026-07-13 wave-1.5 全落地:I `c2a98dfb5`(wallet_fill_observations,foreign/ambiguous 永久保留)· E2 `87bbd895d`(**C1 关闭** — EDLI fill → canonical venue_trade_facts,source-hash 幂等,投影并存至 LX-3R)· H `499b20e0c`(四硬化修;manifest --check 行号不敏感后首次在 session tip exit 0)· F2 `2794f8bb0`(**C2 关闭** — POSITION_IDENTITY_SUPERSEDED + T5 型 live-DB CHECK 迁移 `scripts/migrations/2026_07_position_identity_supersession_check.py` + stale-binary 容错 fallback;F2 判原 revert 因 = 无迁移路径即碰已部署 DB 的 CHECK)。集成:11 测试面 286/286 绿;boot ALL PASS(wallet_fill_observations 经 ensure_table 补建 live DB);manifest 15→14 写者(_merge_equivalent_rows 依设计退役)。**操作员待办:RED 窗口跑 position_identity_supersession CHECK 迁移 → 之后 backfill_identity_supersession_facts --apply(迁移前 fail-closed 拒跑)。**
- 2026-07-13 LX-2R-a 落地 `2e96c639b`:src/reduce/ 目标命名空间 — position_economics 纯确定性 reducer(消费 fill_dedup CTE/supersession 事实/payout_observations,加权成本折算,PENDING 永不为零)+ generation/coverage-vector 契约(全或无发布)+ 5 类 typed refusal(watermark 缺失/CHECK 未迁移/归属缺失/未知 intent/超卖)全 fail-closed。27/27 绿,manifest --check 保持绿(reducer 只读,零新写者)。
- 2026-07-13 ①完成(live DB 核实):position_events CHECK 收 POSITION_IDENTITY_SUPERSEDED、supersession backfill 落 1 fact。
- 2026-07-13 LX-2R-b 落地 `436b528f8`(cherry-pick 自 agent 059b699ee,新命名空间零碰撞):`src/reduce/condition_resolver.py`(condition_id + direction 约定 buy_yes→idx0/buy_no→idx1,411/411 对世界结算验证过;market_id/ctf_token_registry fallback 实测拒绝并存档理由)+ `src/reduce/materialize.py`(`materialize_generation`,keeper 去重防 supersession 三计,经既有 GenerationStore 全或无发布)。45/45 绿。**cent-equivalence 门只读证明(live DB 表级导出副本,零触 live 文件)= 链上真相记分牌:realized −$186.31(held-to-settlement −$201.61)、win-rate 52.1% < 55.7% price 保本 = 链上实证 −EV;编造列逐仓绝对误差 $553.74、63 settled 结果被记 $0(Wellington 6f92d690-3e6 链上 −$97.06 记成 $0.002)。覆盖 954/970(98.4%);16 拒 = 100% OversoldPositionError = 缺 ENTRY fill 覆盖(LX-1R backfill gap,含 supersession 组 chain 60 vs local 44)。** reducer/generation 仍 inert(物化 runner 未对 live 跑)。
- LX-3R firewall scope 已测绘(`scratchpad/residue_firewall_scope.md`):17/40 reader 触 Tier-0;最高危单点 = `query_portfolio_loader_view`(db.py:11612,riskguard hydration);`_void_row` 清零今 load-bearing(无 runtime reader 消费 supersession fact,只 reducer)→ reader 先切 writer 后停;manifest 漏数(SELECT * reader + 两 fill 原语 + edli_live_profit_audit promotion_eligible 读点)须先补全;半数 writer/reader 落操作员活跃编辑文件 → cutover 协调不绕行。
- 2026-07-14 ①+② 执行 + 双 correctness bug 修复(全 live 验证):
  - ① fill synchronizer 启用(config `edli.fill_synchronizer_enabled: true` + price-channel-ingest 重启),watermark(`polymarket_v2_get_trades`,1116 scan/0 append=dedup 干净)+ wallet_fill_observations(1116,FOREIGN 772/ZEUS 344)落。
  - ② 对 live 跑 materialize → generation `b84c3f8b`,链上真相 **−$200 / 26 refusal**。
  - **live 物化揭出两个真 bug(编造列从不暴露,链上真相才暴露):** (a) reducer 按 observed_at(摄入时序)折叠 → 22:02 REST re-confirm 把 entry 的 observed_at 重戳到其 exit 之后 → 假 OversoldPositionError(Wellington −$97 等 ~11 仓被误拒)。修 `30abcff39`:按 execution_ts(venue_timestamp,缺则 revision 内最早 observed_at)折叠;fill_dedup CTE 加 execution_ts 加法列(不改 canonical 选择,exchange_reconcile 86 基线不变);回归测试锁定。 (b) synchronizer append_trade_fact 丢 venue_timestamp(只 observation lane 存)→ 其 fills 无执行时序。修 `7e4ee1b4a`(操作员 test 微调 `6698c8ae2`)。
  - manifest `--check` 曾因操作员动态 `table_name` 隐掉 funnel writer 而 RED → 修 `7ce76bcd3`(scanner 补动态表名 + SELECT* reader;14 writer/54 reader,green)。
  - **LX-3R cutover foundation 落地 `ddaf15fc0`(staged,零 armed):** `src/reduce/read_model.py`(`latest_position_economics`:keeper 解析 + None=UNKNOWN-never-zero,readers cutover 时消费此)+ `materialize_and_publish_cycle` + `scripts/migrations/2026_07_economics_column_firewall.py`(BEFORE INSERT/UPDATE 触发器冻结禁列;**edli_live_profit_audit 在 zeus-world.db 跨库→T5 ATTACH 原子**;promotion_eligible NOT NULL DEFAULT 0 → 只拦 `<>0` 肯定声明)。99 测试绿,未 apply、未接 daemon、未触操作员 money-path 文件。
  - **有界残留:** ~3 历史仓(22:02 backlog sweep 的 null-vts fragment,如 e3ce031a)仍 refuse;synchronizer 修阻止复发,回填低值(venue_trade_facts immutability trigger→需 supersession 机制)暂缓。
  - 操作员并行动作:retire/demote deployment-freshness gates(`d0ea2d16a`/`079b0ad26`)→ SHA drift 不再停 entries。
- NEXT = **单次 fenced atomic cutover**(必须一起落地+激活,partial land 危险:reader 读到 stale/incomplete read-model 会误导 riskguard):接 `materialize_and_publish_cycle` 进 daemon → readers 切 `read_model.latest_position_economics`(None fail-closed)→ 收窄 funnel(projection.py)+ 停 14 writer → arm `economics_column_firewall`(--operator-confirms-fenced,writer 收窄后)→ flip truth_epoch ACTIVE_NEW → deploy。半数落操作员活跃文件(command_recovery/exchange_reconcile/projection)→ 待其 money-path 工作 settle 后协调同步。→ LX-4R 隔离 7d → LX-5R drop。
