...
The σ=0.8 spike-into-an-impossible-bin the operator saw is therefore the **EDLI-maze lane's σ** (`state/sigma_scale_fit.json` k=1.58, w=0.28 — and note this artifact is `candidate:true`, OPERATOR_GATED, holdout "non-stationary on 5 days"), compounding the R3 +4.847°C center error. On the live EMOS lane this specific spike cannot occur (σ≈2.0).
