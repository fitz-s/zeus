...
| P2-H | P1 | Forecast-layer Phase-1 de-hardcode: Day0 backbone, lead-continuous mean/sigma, heteroscedastic sigma | forecast lane | IN_PROGRESS | Low-blast-radius seam work is underway: forecast uncertainty policy is centralized behind `src/signal/forecast_uncertainty.py`; day0 sigma + observation-weight + residual-blend policy are extracted; and the analysis sigma seam now carries `lead_days` and `ensemble_spread` without changing current behavior. Next work in this lane is the first behavior-changing forecast de-hardcode step. |
