...
- Live follow-up after PR #288: `imminent_open_capture` still produced `settlement_capture` economic-floor rows for high bins above current observed high. The Day0/nowcast semantic split must apply to imminent settlement-day capture as well as `day0_capture`; otherwise Jeddah/Chicago forecast-upside edges keep masquerading as observation-locked settlement capture.
- Live follow-up after PR #289: code-plane and health are green, but opening_hunt now rejects candidates as `CROSSCHECK_UNAVAILABLE` with `SOURCE_COMPARABILITY_FAILED` because Open-Meteo GFS025 diagnostic payloads do not expose true issue_time. Current branch allows that source-limited crosscheck only when target-day valid windows match and the diagnostic fetch/capture time is fresh relative to primary issue; it still refuses stale, shifted-window, or non-Open-Meteo missing-issue crosschecks.
